...
class ContinuousMonitor:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        self.config_path = config_path or self.project_root / "config" / "monitoring.yml"
    ...
    def generate_trend_report(self) -> Dict:
        ...
        # 保存趋势报告
        trend_report_path = self.reports_dir / f"trend_report_{datetime.now().strftime('%Y%m%d')}.json"
    ...
    def cleanup_old_reports(self):
        """清理旧报告"""
        retention_days = self.config.get('monitoring', {}).get('reports', {}).get('retention_days', 30)
    ...
    def _generate_monitoring_summary(self, quality_report: Dict, trend_report: Dict) -> str:
        """生成监控摘要"""
        summary_lines = []
