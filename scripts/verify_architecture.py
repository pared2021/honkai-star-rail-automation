...
class ArchitectureValidator:
    ...
    def __init__(self, project_root: Path):
        ...
        self.expected_structure = {
            ...
            "src/application": ["task_service.py", "automation_service.py", "error_handling_service.py"],
        ...
        self.dependency_rules = {
            "ui": ["core", "application", "adapters", "services", "models", "config"],  # UI层
            "application": ["core", "services", "repositories", "models", "config", "exceptions", "database"],  # 应用层
            "services": ["core", "repositories", "models", "config", "exceptions", "database"],  # 服务层
            "repositories": ["core", "models", "config", "database", "exceptions"],  # 仓储层
            "core": ["models", "config", "exceptions", "database", "services"],  # 核心层 - 允许依赖基础设施
            "adapters": ["core", "application", "services", "models", "config"],  # 适配器层
            "automation": ["core", "services", "models", "config", "exceptions"],  # 自动化层
            "monitoring": ["core", "services", "database", "automation", "models", "config"],  # 监控层
    ...
    def _check_layer_dependencies(self, file_path: Path, layer: str, imports: List[str]):
        """检查层级依赖规则"""
            ...
            if import_module.startswith("src."):
                ...
                if len(import_parts) >= 2:
                    ...
                    if imported_layer != layer and imported_layer not in allowed_layers:
                        # 检查是否是允许的特殊情况
                        if not self._is_allowed_exception(layer, imported_layer, import_module):
                            self.issues.append(
                                f"违反依赖规则: {layer}层的{file_path.name}不应依赖{imported_layer}层({import_module})"
                            )
    
    def _is_allowed_exception(self, from_layer: str, to_layer: str, import_module: str) -> bool:
        """检查是否是允许的依赖异常"""
        # 允许的特殊情况
        exceptions = {
            # 服务定位器模式允许的导入
