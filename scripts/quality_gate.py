...
class QualityGate:
    ...
    def run_code_formatting_check(self, fix: bool = False) -> CheckResult:
        ...
        # 综合结果
        if (
            black_result.status == CheckStatus.PASSED
            and isort_result.status == CheckStatus.PASSED
        ):
    ...
    def run_quality_check(self) -> CheckResult:
        ...
        try:
            cmd = (
                self.tools["pylint"]["command"]
                + self.tools["pylint"]["check_args"]
                + [str(self.src_dir)]
            )
    ...
    def run_type_check(self) -> CheckResult:
        ...
        try:
            cmd = (
                self.tools["mypy"]["command"]
                + self.tools["mypy"]["check_args"]
                + [str(self.src_dir)]
            )
    ...
    def run_duplicate_check(self) -> CheckResult:
        ...
        try:
            ...
            cmd = [
                ...
                "--output",
                str(self.reports_dir / "duplicates.json"),
            ]
    ...
    def run_test_coverage_check(self) -> CheckResult:
        ...
        try:
            ...
            # 运行测试和覆盖率检查
            cmd = [sys.executable, str(test_script), "run", "--format", "junit"]
            ...
            coverage_cmd = [
                ...
                "check",
                "--threshold",
                str(self.thresholds["coverage_threshold"]),
            ]

            coverage_result = subprocess.run(
                coverage_cmd, cwd=self.project_root, capture_output=True, text=True
            )
            ...
            if result.returncode == 0 and coverage_result.returncode == 0:
                ...
                status = CheckStatus.FAILED
                message = (
                    f"测试覆盖率未达到阈值 {self.thresholds['coverage_threshold']}%"
                )
    ...
    def run_all_checks(self, fix: bool = False, skip_tests: bool = False) -> bool:
        ...
        for check_name, check_func in checks:
            try:
                ...
                print(
                    f"{status_icon} {result.name}: {result.message} ({result.duration:.1f}s)"
                )
    ...
    def _generate_report(self) -> None:
        ...
        report_data = {
            ...
            "summary": {
                ...
                "passed": len(
                    [r for r in self.results if r.status == CheckStatus.PASSED]
                ),
                "failed": len(
                    [r for r in self.results if r.status == CheckStatus.FAILED]
                ),
                "skipped": len(
                    [r for r in self.results if r.status == CheckStatus.SKIPPED]
                ),
                "errors": len(
                    [r for r in self.results if r.status == CheckStatus.ERROR]
                ),
...
def main():
    ...
    parser.add_argument("--skip-tests", action="store_true", help="跳过测试覆盖率检查")
    ...
    try:
        # 运行检查
        success = gate.run_all_checks(fix=args.fix, skip_tests=args.skip_tests)
