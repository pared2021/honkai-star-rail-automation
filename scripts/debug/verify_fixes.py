...
def test_task_manager_import():
    ...
    try:
        ...
        from models.task_models import TaskConfig, TaskPriority, TaskType
...
def test_get_task_sync_method():
    ...
    try:
        ...
        # 检查方法是否存在
        assert hasattr(
            TaskManagerAdapter, "get_task_sync"
        ), "TaskManagerAdapter 缺少 get_task_sync 方法"

        # 检查方法是否可调用
        assert callable(
            getattr(TaskManagerAdapter, "get_task_sync")
        ), "get_task_sync 不是可调用方法"
