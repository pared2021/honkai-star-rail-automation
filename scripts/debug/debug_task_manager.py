...
try:
    ...
    try:
        ...
        if hasattr(task_manager, "get_task_sync"):
            ...
            instance_methods = [
                attr
                for attr in dir(task_manager)
                if not attr.startswith("_") and callable(getattr(task_manager, attr))
            ]
            get_task_instance_methods = [
                method for method in instance_methods if "get_task" in method
            ]
