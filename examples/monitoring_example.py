...
class MonitoringExampleApp:
        ...
        class TaskProcessorHealthCheck(BaseHealthCheck):
            ...
            def _perform_check(self):
                try:
                    ...
                    if last_processed and (datetime.now() - last_processed).seconds > 300:
    ...
    def _simulate_normal_operations(self):
        ...
        for i in range(10):
            ...
            # 模拟游戏检测
            if i % 3 == 0:
                self.event_bus.emit('game_detected', {
                    'game_name': 'TestGame',
    ...
    def _simulate_error_conditions(self):
        ...
        # 模拟系统错误
        self.event_bus.emit('system_error', {
        ...
        # 模拟自动化错误
        self.event_bus.emit('automation_error', {
    ...
    def _simulate_performance_issues(self):
        ...
        # 模拟性能警告
        self.event_bus.emit('performance_warning', {
    ...
    def _export_monitoring_data(self):
        """导出监控数据"""
        export_file = f"monitoring_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
