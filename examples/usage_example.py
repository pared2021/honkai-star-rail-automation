...
from src.core.models.task import Task, TaskConfig, TaskType, TaskStatus, TaskPriority
...
async def example_event_handling():
    ...
    async def on_task_status_changed(event: TaskStatusChangedEvent):
        logger.info(f"任务状态变更: {event.task_id} - {event.old_status} -> {event.new_status}")
...
async def example_cache_usage():
    ...
    # 缓存数据
    await cache_manager.set("user:123:profile", {
        "name": "张三",
    ...
    # 缓存任务结果
    await cache_manager.cache_task_result("task123", {
        "status": "completed",
        "result": "处理完成",
...
async def example_resource_monitoring():
    ...
    # 分配资源
    allocation = await resource_manager.allocate_resources("task123", {
        'cpu': 2.0,
        'memory': 1024,
        'disk': 500
    })
