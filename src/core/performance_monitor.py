...
class PerformanceOptimizer:
    ...
    def optimize_template_matching(
        self, templates: List[str], performance_level: PerformanceLevel
    ) -> List[str]:
        """优化模板匹配"""
        if performance_level in [PerformanceLevel.POOR, PerformanceLevel.CRITICAL]:
            # 限制同时处理的模板数量
            max_templates = (
                self.max_templates_per_detection // 2
                if performance_level == PerformanceLevel.CRITICAL
                else self.max_templates_per_detection
            )
    ...
    def _apply_auto_optimization(self, performance_level: PerformanceLevel):
        ...
        if performance_level in [PerformanceLevel.POOR, PerformanceLevel.CRITICAL]:
            ...
            if strategies_applied and self.optimization_applied_callback:
                self.optimization_applied_callback(
                    strategies_applied, performance_level
                )
        ...
        summary = {
            ...
            "image_processing": {
                ...
                "average_template_time": (
                    self.image_metrics.template_match_time
                    / self.image_metrics.templates_processed
                    if self.image_metrics.templates_processed > 0
                    else 0
                ),
            },
            ...
            recommendations.append(
                {
                    ...
                    "level": (
                        "warning"
                        if current_metrics.memory_percent
