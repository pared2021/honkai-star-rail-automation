...
class TemplateMatcher:
    ...
    def load_templates(self, templates_dir: str) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Invalid templates directory: {templates_dir}"
            )
            return

        if not os.path.exists(templates_dir_str):
            self.logger.warning(
                f"Templates directory not found: {templates_dir_str}"
            )
            return
        ...
        try:
            ...
            if template_path.lower().endswith(".svg"):
                ...
                if not CAIROSVG_AVAILABLE:
                    self.logger.warning(
                        f"cairosvg not available, skipping SVG template: "
                        f"{template_path}"
                    )
                    return None
                try:
                    ...
                    if image is None:
                        self.logger.warning(
                            f"Failed to convert SVG template: {template_path}"
                        )
                        return None
                except Exception as e:
                    self.logger.warning(
                        f"Error reading SVG file {template_path}: {e}"
                    )
                    return None
            else:
                # 普通图像文件
    ...
    def _convert_svg_to_image(
        self, svg_content: Union[str, Path]
    ) -> Optional[np.ndarray]:
        """将SVG内容转换为图像.

        Args:
            svg_content: SVG文件内容或路径.
        ...
        try:
            ...
            if image_pil.mode != "RGB":
                image_pil = image_pil.convert("RGB")  # type: ignore

            # 转换为numpy数组
            image_array = np.array(image_pil)

            # 转换为BGR格式（OpenCV使用BGR）
            image_bgr: np.ndarray = cv2.cvtColor(
    ...
    def _calculate_scale_factors(
        ...
        # 如果模板太大，添加更小的缩放因子
        if (template_w > screenshot_w * 0.5 or
                template_h > screenshot_h * 0.5):
            scale_factors.extend([0.5, 0.6, 0.7])

        # 如果模板很小，添加更大的缩放因子
        if (template_w < screenshot_w * 0.1 and
                template_h < screenshot_h * 0.1):
            scale_factors.extend([1.3, 1.4, 1.5])

        return sorted(set(scale_factors))

    def _scale_template(
        self, template: np.ndarray, scale_factor: float
    ) -> np.ndarray:
        """缩放模板图像.
    ...
    def match_template(
        self, screenshot: np.ndarray, template_name: Union[str, TemplateInfo]
    ) -> Optional[UIElement]:
        """在截图中匹配模板.
        ...
        try:
            ...
            result = cv2.matchTemplate(
                screenshot, template_image, cv2.TM_CCOEFF_NORMED
            )
    ...
    def match_multiple_templates(
        self, screenshot: np.ndarray, template_names: List[str]
    ) -> List[UIElement]:
        """匹配多个模板。."""
        results = []
...
class WindowManager:
    ...
    def capture_window(self, game_window: GameWindow) -> Optional[np.ndarray]:
        ...
        try:
            ...
            img_bgr: np.ndarray = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)
...
class GameDetector:
    ...
    def wait_for_ui_element(
        self, template_name: str, timeout: float = 10.0, interval: float = 0.5
    ) -> Optional[UIElement]:
        """等待UI元素出现。."""
    ...
    def is_game_running(self) -> bool:
        ...
        with contextlib.suppress(Exception):
            if win32gui:
                result: bool = (win32gui.IsWindow(window.hwnd) and
                                win32gui.IsWindowVisible(window.hwnd))
                return result
    ...
    def detect_current_scene(self) -> SceneType:
        ...
        for scene_type, templates in scene_templates.items():
            ...
            if elements:
                # 使用最高置信度作为场景置信度
                max_confidence = max(
                    element.confidence for element in elements
                )
    ...
    def detect_and_visualize_scene(
        ...
        elements = self.template_matcher.match_multiple_templates(
            screenshot, all_templates
        )

        return self.visualize_detection_results(
            screenshot, elements, save_path
        )
