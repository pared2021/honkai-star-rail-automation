...
class TemplateMatcher:
    ...
    def match_template(
        ...
        if scale_factors is None:
            scale_factors = self._calculate_scale_factors(
                screenshot.shape[:2], template_info["original_size"]
            )
        ...
        try:
            ...
            if best_match is None:
                logger.debug(
                    f"Template {template_name} not found, best confidence: {best_confidence:.3f}"
                )
    ...
    def _calculate_scale_factors(
        self, screenshot_size: Tuple[int, int], template_size: Tuple[int, int]
    ) -> List[float]:
        """计算适合的缩放因子"""
        ...
        for scale in base_scales:
            ...
            # 确保缩放后的模板不会超出屏幕或太小
            if 10 <= scaled_w <= screen_w - 10 and 10 <= scaled_h <= screen_h - 10:
                valid_scales.append(scale)

        return valid_scales if valid_scales else [1.0]

    def _scale_template(
        self, template: np.ndarray, scale: float
    ) -> Optional[np.ndarray]:
        """缩放模板图像"""
        if scale == 1.0:
            return template
    ...
    def match_multiple_templates(
        self, screenshot: np.ndarray, template_names: List[str], threshold: float = None
    ) -> List[UIElement]:
        """匹配多个模板"""
        results = []
...
class GameDetector:
    ...
    def __init__(
        self, config_manager: ConfigManager, game_title: str = "崩坏：星穹铁道"
    ):
    ...
    def detect_current_scene(
        self, screenshot: Optional[np.ndarray] = None, confidence_threshold: float = 0.7
    ) -> SceneType:
        """检测当前游戏场景，使用增强的检测逻辑"""
        ...
        for scene_type, template_names in self.scene_templates.items():
            ...
            if matches:
                # 计算场景分数：匹配数量 * 平均置信度
                avg_confidence = sum(match.confidence for match in matches) / len(
                    matches
                )
                ...
                logger.debug(
                    f"Scene {scene_type.value}: {len(matches)} matches, "
        ...
        if scene_scores:
            ...
            logger.info(
                f"Detected scene: {best_scene[0].value} (score: {best_scene[1]:.3f})"
            )
    ...
    def find_ui_element(
        self, template_name: str, screenshot: Optional[np.ndarray] = None
    ) -> Optional[UIElement]:
        """查找UI元素"""
        if screenshot is None:
            screenshot = self.capture_screenshot()
    ...
    def find_multiple_ui_elements(
        self, template_names: List[str], screenshot: Optional[np.ndarray] = None
    ) -> List[UIElement]:
        """查找多个UI元素"""
        if screenshot is None:
            screenshot = self.capture_screenshot()
        ...
        return self.template_matcher.match_multiple_templates(
            screenshot, template_names
        )

    def wait_for_ui_element(
        self, template_name: str, timeout: float = 10.0, check_interval: float = 0.5
    ) -> Optional[UIElement]:
        """等待UI元素出现"""
    ...
    def is_game_running(self) -> bool:
        ...
        try:
            # 检查窗口是否仍然有效
            return win32gui.IsWindow(window.hwnd) and win32gui.IsWindowVisible(
                window.hwnd
            )
        except Exception:
            return False
    ...
    def get_game_status(self) -> Dict:
        ...
        return {
            ...
            "current_scene": (
                self.current_scene.value if self.current_scene else "unknown"
            ),
    ...
    def visualize_detection_results(
        ...
        for element in elements:
            ...
            # 绘制文本背景
            cv2.rectangle(
                result_image, (x, y - text_h - 10), (x + text_w, y), (0, 0, 0), -1
            )
    ...
    def detect_and_visualize_scene(
        ...
        # 可视化结果
        result_image = self.visualize_detection_results(
            screenshot, all_elements, save_path
        )
