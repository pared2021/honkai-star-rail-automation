...
class ExceptionDetector:
    ...
    def detect_game_freeze(self) -> bool:
        ...
        # 如果场景长时间未变化，可能是冻结
        if (
            datetime.now() - self.last_scene_change
        ).seconds > self.scene_stability_threshold:
            # 尝试点击检测响应
...
class ExceptionRecovery:
    ...
    def handle_exception(self, exception_info: ExceptionInfo) -> RecoveryResult:
        """处理异常"""
        logger.warning(
            f"检测到异常: {exception_info.exception_type.value} - {exception_info.description}"
        )
        ...
        if not strategy:
            logger.error(
                f"未找到异常类型的恢复策略: {exception_info.exception_type.value}"
            )
            return RecoveryResult(
                success=False,
                actions_taken=[],
    ...
    def _execute_recovery(
        self, strategy: RecoveryStrategy, exception_info: ExceptionInfo
    ) -> RecoveryResult:
        """执行恢复策略"""
        start_time = time.time()
        actions_taken = []
    ...
    def _refresh_page(self) -> bool:
        ...
        return self.automation_controller.execute_action(
            {"type": ActionType.KEY_PRESS.value, "key": "F5"}
        )
    ...
    def _return_to_main(self) -> bool:
        ...
        for _ in range(3):
            success = self.automation_controller.execute_action(
                {"type": ActionType.KEY_PRESS.value, "key": "Escape"}
            )
            if not success:
                return False
            time.sleep(1)
    ...
    def _force_close_game(self) -> bool:
        ...
        try:
            ...
            for proc in game_processes:
                ...
                logger.info(
                    f"已终止游戏进程: {proc.info['name']} (PID: {proc.info['pid']})"
                )
    ...
    def get_exception_statistics(self) -> Dict[str, Any]:
        ...
        if self.recovery_history:
            avg_recovery_time = sum(r.time_taken for r in self.recovery_history) / len(
        ...
        return {
            ...
            "last_exception": (
                self.exception_history[-1].timestamp.isoformat()
    ...
    def set_callbacks(
        self,
        exception_detected: Optional[Callable] = None,
