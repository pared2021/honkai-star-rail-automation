...
from .enums import ActionType, TaskPriority, TaskType
...
class TaskValidator:
    ...
    def _validate_single_action(
        self, action_data: Dict[str, Any], index: int
    ...
    def _validate_action_params(
        self, action_type: ActionType, params: Dict[str, Any], field_prefix: str
    ) -> List[ValidationResult]:
        """验证操作参数值"""
        results = []
        ...
        if "duration" in params:
            if (
                not isinstance(params["duration"], (int, float))
        ...
        if "timeout" in params:
            if (
                not isinstance(params["timeout"], (int, float))
                or params["timeout"] <= 0
            ):
                results.append(
                    ValidationResult(
