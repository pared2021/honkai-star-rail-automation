...
class RetryConfiguration:
    ...
    # 触发条件配置
    triggers: List[RetryTrigger] = field(default_factory=lambda: [
...
class TaskRetryManager(QObject):
    ...
    def schedule_retry(self, 
        ...
        try:
            ...
            logger.info(f"已调度任务 {task_id} 重试，第 {attempt.attempt_number} 次尝试，延迟 {delay:.2f} 秒")
            return True
    ...
    def cancel_retry(self, task_id: str, reason: str = "用户取消") -> bool:
        ...
        try:
            # 从队列中移除
            self.retry_queue = [
                (tid, time) for tid, time in self.retry_queue 
                if tid != task_id
            ]
    ...
    def mark_retry_result(self, task_id: str, success: bool, error_message: Optional[str] = None):
        """标记重试结果"""
        ...
        if context.attempts:
            ...
            if success:
                ...
                logger.warning(f"任务 {task_id} 重试次数用尽，共尝试 {context.total_attempts} 次")
    ...
    def get_retry_status(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "next_retry_time": context.next_retry_time.isoformat() if context.next_retry_time else None,
            "final_result": context.final_result,
            "last_error": context.attempts[-1].error_message if context.attempts else None,
    ...
    def get_all_retry_status(self) -> List[Dict[str, Any]]:
        """获取所有任务的重试状态"""
        return [
            self.get_retry_status(task_id)
            for task_id in self.retry_contexts.keys()
        ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if total_contexts > 0:
            total_attempts = sum(ctx.total_attempts for ctx in self.retry_contexts.values())
    ...
    def _get_or_create_context(self, 
                              ...
                              task_name: str,
                              user_id: Optional[str],
                              config: Optional[RetryConfiguration]) -> TaskRetryContext:
        """获取或创建重试上下文"""
        if task_id not in self.retry_contexts:
            # 获取配置
            effective_config = (
                config or 
                self.task_configs.get(task_id) or 
                self.default_config
            )
    ...
    def _can_retry(self, context: TaskRetryContext, trigger: RetryTrigger) -> bool:
        ...
        if context.last_attempt_time:
            ...
            time_since_last = (datetime.now() - context.last_attempt_time).total_seconds()
    ...
    def _execute_retry(self, task_id: str):
        ...
        try:
            ...
            logger.info(f"开始执行任务 {task_id} 的第 {current_attempt.attempt_number} 次重试")
    ...
    def cleanup_completed_contexts(self, max_age_hours: int = 24):
        ...
        for task_id, context in self.retry_contexts.items():
            if (not context.is_active and 
                context.start_time < cutoff_time):
