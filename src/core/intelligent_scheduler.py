...
class IntelligentScheduler:
    ...
    def __init__(self, task_manager: TaskManager, game_detector: GameDetector, 
        ...
        self.scene_task_mapping = {
            ...
            SceneType.GAME_WORLD: [TaskType.EXPLORATION, TaskType.DAILY_MISSION, TaskType.COMBAT_TRAINING],
    ...
    def add_task(self, task: Task, priority: Optional[int] = None, 
    ...
    def _can_execute_task(self, scheduled_task: ScheduledTask) -> bool:
        ...
        # 检查重试延迟
        if (scheduled_task.last_attempt and 
            datetime.now() - scheduled_task.last_attempt < timedelta(seconds=self.config.retry_delay)):
            return False
    ...
    def _check_dependencies(self, scheduled_task: ScheduledTask) -> bool:
        ...
        for dep_task_id in scheduled_task.dependencies:
            if dep_task_id not in self.completed_tasks:
                logger.debug(f"任务 {scheduled_task.task.task_id} 等待依赖任务 {dep_task_id} 完成")
                return False
        return True
    ...
    def _run_task_actions(self, task: Task) -> TaskExecutionResult:
        ...
        try:
            ...
            for action_data in task.config.get('actions', []):
                ...
                # 执行单个操作
                success = self.automation_controller.execute_action_from_dict(action_data)
                if not success:
    ...
    def set_callbacks(self, task_started: Optional[Callable] = None,
