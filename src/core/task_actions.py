...
class BaseAction(ABC):
    ...
    def post_execute(self, success: bool, error_message: str = None):
        ...
        if self.result:
            ...
            self.result.status = ActionStatus.COMPLETED if success else ActionStatus.FAILED
            if error_message:
                self.result.error_message = error_message
            
            logger.info(f"动作执行完成: {self.action_type.value} - {self.result.status.value}")


class ClickAction(BaseAction):
    """点击动作"""
    
    def __init__(self, action_id: str, x: int, y: int, button: str = 'left', 
...
class KeyPressAction(BaseAction):
    ...
    def __init__(self, action_id: str, key: str, presses: int = 1, interval: float = 0.0):
...
class ScreenshotAction(BaseAction):
    ...
    def __init__(self, action_id: str, save_path: str = None, region: Tuple[int, int, int, int] = None):
...
class CustomAction(BaseAction):
    ...
    def __init__(self, action_id: str, script: str, script_params: Dict[str, Any] = None):
...
class ActionFactory:
    ...
    def create_action(action_data: Dict[str, Any]) -> BaseAction:
        ...
        if action_type == ActionType.CLICK.value:
            ...
            return WaitAction(
                action_id=action_id,
                duration=action_data.get('wait_duration', 1.0)
            )
