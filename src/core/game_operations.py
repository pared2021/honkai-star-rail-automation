...
class GameOperations:
    ...
    def _safe_click(
        self, element: UIElement, click_type: ClickType = ClickType.SINGLE
    ) -> bool:
        """安全点击UI元素"""
        try:
            # 添加随机偏移
            ...
            result = self.action_executor.execute_click(
                position=(click_x, click_y), click_type=click_type
            )
    ...
    def _wait_for_scene_change(
        self, target_scene: SceneType, timeout: float = 10.0
    ) -> bool:
        """等待场景切换"""
    ...
    async def execute_daily_stamina_consumption(
        ...
        try:
            ...
            # 选择材料本
            material_dungeon = self.game_detector.find_ui_element(
                "activity_material_dungeon"
            )
            if material_dungeon:
            ...
            while consumed_stamina < target_stamina and attempts < max_attempts:
                ...
                # 查找挑战按钮
                challenge_button = self.game_detector.find_ui_element(
                    "dungeon_challenge_button"
                )
                ...
                # 检查体力是否足够
                insufficient_stamina = self.game_detector.find_ui_element(
                    "insufficient_stamina_dialog"
                )
                if insufficient_stamina:
                    if auto_use_items:
                        # 尝试使用体力道具
                        use_item_button = self.game_detector.find_ui_element(
                            "use_stamina_item_button"
                        )
                        if use_item_button:
                            self._safe_click(use_item_button)
                            self._random_delay(1.0, 2.0)

                            confirm_button = self.game_detector.find_ui_element(
                                "confirm_button"
                            )
                            if confirm_button:
                ...
                # 开始战斗
                start_battle_button = self.game_detector.find_ui_element(
                    "start_battle_button"
                )
                if start_battle_button:
                    ...
                    # 等待战斗结束
                    battle_end = self.game_detector.wait_for_ui_element(
                        "battle_result_dialog", timeout=120.0
                    )
                    if battle_end:
            ...
            if consumed_stamina >= target_stamina:
                return TaskResult(
                    ...
                    duration=duration,
                    details={
                        "consumed_stamina": consumed_stamina,
                        "attempts": attempts,
                    },
                )
            else:
    ...
    async def execute_daily_missions(self, config: Dict[str, Any]) -> TaskResult:
        ...
        try:
            ...
            priority_order = config.get(
                "priority_order", ["dispatch", "assignment", "synthesis"]
            )
            ...
            if auto_claim:
                ...
                # 一键领取
                claim_all_button = self.game_detector.find_ui_element(
                    "mission_claim_all"
                )
                if claim_all_button:
    ...
    async def _execute_dispatch_missions(self) -> bool:
        ...
        try:
            ...
            for slot in dispatch_slots:
                ...
                # 选择角色
                character_slot = self.game_detector.find_ui_element(
                    "dispatch_character_slot"
                )
                if character_slot:
                ...
                # 确认派遣
                confirm_dispatch = self.game_detector.find_ui_element(
                    "dispatch_confirm_button"
                )
                if confirm_dispatch:
    ...
    async def _execute_assignment_missions(self) -> bool:
        ...
        try:
            # 打开委托界面
            assignment_button = self.game_detector.find_ui_element(
                "mission_assignment_tab"
            )
    ...
    async def _execute_synthesis_missions(self) -> bool:
        ...
        try:
            # 打开合成界面
            synthesis_button = self.game_detector.find_ui_element(
                "main_synthesis_button"
            )
            if synthesis_button:
            ...
            for item in synthesis_items:
                ...
                # 确认合成
                craft_button = self.game_detector.find_ui_element(
                    "synthesis_confirm_button"
                )
                if craft_button:
    ...
    async def execute_auto_combat(self, config: Dict[str, Any]) -> TaskResult:
        ...
        try:
            ...
            if auto_battle_enabled:
                auto_battle_button = self.game_detector.find_ui_element(
                    "combat_auto_battle"
                )
            ...
            # 等待战斗结束
            battle_end = self.game_detector.wait_for_ui_element(
                "battle_result_dialog", timeout=300.0
            )
    ...
    async def execute_mail_collection(self, config: Dict[str, Any]) -> TaskResult:
        ...
        try:
            ...
            # 如果启用自动删除
            if auto_delete:
                delete_all_button = self.game_detector.find_ui_element(
                    "mail_delete_all"
                )
                if delete_all_button:
                    ...
                    # 确认删除
                    confirm_delete = self.game_detector.find_ui_element(
            ...
            return TaskResult(
                ...
                duration=duration,
                details={
                    "collected_mails": collected_mails,
