...
from core.task_retry_manager import TaskRetryManager, RetryTrigger
from application.error_handling_service import ErrorHandlingService, ErrorContext
from config.error_recovery_strategies import error_recovery_config, ErrorRecoveryStrategy
...
class ErrorRecoveryCoordinator:
    ...
    async def coordinate_recovery(self, 
        ...
        # 发送事件
        self.event_bus.emit('recovery_session_created', {
    ...
    async def _create_recovery_session(self, 
                                     error: Exception, 
                                     context: ErrorContext) -> RecoverySession:
        """创建恢复会话"""
    ...
    def _create_default_strategy(self, error_type: str) -> ErrorRecoveryStrategy:
        """创建默认恢复策略"""
        from config.error_recovery_strategies import ErrorRecoveryStrategy, ErrorCategory
    ...
    async def _retry_recovery(self, session: RecoverySession):
        """重试恢复"""
        delay = session.strategy.initial_retry_delay * (session.strategy.retry_backoff_factor ** (session.attempt_count - 1))
    ...
    async def _escalate_session(self, session: RecoverySession):
        ...
        # 发送升级事件
        self.event_bus.emit('recovery_escalated', {
            'session_id': session.session_id,
            'error_type': session.error_type,
    ...
    async def _change_phase(self, session: RecoverySession, phase: RecoveryPhase):
        ...
        self.logger.debug(f"会话 {session.session_id} 阶段变更: {old_phase.value} -> {phase.value}")
    ...
    def _update_average_recovery_time(self, recovery_time: float):
        """更新平均恢复时间"""
        total_completed = self.stats['successful_recoveries'] + self.stats['failed_recoveries']
        if total_completed > 0:
            current_avg = self.stats['average_recovery_time']
            self.stats['average_recovery_time'] = (
                (current_avg * (total_completed - 1) + recovery_time) / total_completed
            )
    ...
    async def _check_session_timeouts(self):
        """检查会话超时"""
        timeout_threshold = datetime.now() - timedelta(seconds=self.config['session_timeout'])
