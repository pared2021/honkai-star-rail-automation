...
class ConcurrentTaskQueue:
    ...
    def __init__(self):
        """初始化队列。"""
        self.queues = {priority: PriorityQueue() for priority in TaskPriority}
    ...
    def put(self, task_execution: TaskExecution) -> None:
        ...
        with self._lock:
            ...
            # 使用任务创建时间作为次要排序键
            priority_queue.put((task_execution.priority.value, time.time(), task_execution))
            self.task_count += 1
...
class TaskManager:
    ...
    async def create_task(
        ...
        **options
        ...
        # 提交到并发队列
        execution_id = self.submit_concurrent_task(
            task_id=task_id,
    ...
    async def list_tasks(self, status: Optional[TaskState] = None) -> List[Dict[str, Any]]:
        ...
        # 收集所有任务
        all_executions = list(self._active_executions.values()) + list(self._completed_executions.values())
    ...
    async def get_task_statistics(self) -> Dict[str, int]:
        ...
        stats.update({
            "created": len(self._active_executions) + len(self._completed_executions),
