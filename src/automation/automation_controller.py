...
class AutomationController:
    ...
    def stop_task(self) -> bool:
        ...
        if self.current_task_id:
            ...
            self.db_manager.add_execution_log(self.current_task_id, "INFO", "任务已停止")
    ...
    def pause_task(self) -> bool:
        ...
        if self.current_task_id:
            self.db_manager.add_execution_log(self.current_task_id, "INFO", "任务已暂停")
    ...
    def resume_task(self) -> bool:
        ...
        if self.current_task_id:
            self.db_manager.add_execution_log(self.current_task_id, "INFO", "任务已恢复")
    ...
    def _execute_task(self):
        ...
        # 更新数据库
        if self.current_task_id:
            self.db_manager.update_task_status(self.current_task_id, self.task_status.value)
    ...
    def _execute_action(self, action: AutomationAction) -> bool:
        ...
        try:
            ...
            if self.current_task_id:
                self.db_manager.add_execution_log(
                    self.current_task_id, "ERROR", 
                    f"操作失败: {action.description}", str(e)
                )
            return False
