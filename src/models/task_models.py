...
class TaskConfig(BaseModel):
    ...
    priority: int = Field(default=TaskPriority.MEDIUM.value, description="任务优先级")
    ...
    automation_config: Optional[Dict[str, Any]] = Field(
        default=None, description="自动化配置"
    )
...
class Task(BaseModel):
    ...
    def validate_config(cls, v):
        ...
        if isinstance(v, TaskConfig):
            # 如果是TaskConfig对象，转换为字典
            return v.to_dict()
        elif isinstance(v, dict):
            return v
        else:
            raise ValueError(f"config must be TaskConfig or dict, got {type(v)}")
    ...
    def update_status(
        self, new_status: TaskStatus, error_msg: Optional[str] = None
    ) -> None:
        """更新任务状态"""
        self.status = (
    ...
    def can_retry(self) -> bool:
        """检查是否可以重试"""
        return self.retry_count < self.config.get("max_retry_count", 3)
    ...
    def priority(self):
        """获取任务优先级"""
        return self.config.get("priority", 2)
...
def create_task(
    config: Union[TaskConfig, Dict[str, Any]], user_id: str = "default_user", **kwargs
) -> Task:
    """创建任务的便利函数"""
    # 如果config是TaskConfig对象，转换为字典
    if isinstance(config, TaskConfig):
        config = config.to_dict()
    return Task(config=config, user_id=user_id, **kwargs)
