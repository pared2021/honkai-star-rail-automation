...
from PyQt6.QtWidgets import (
    ...
    QHeaderView, QPushButton, QLabel, QGroupBox, QMessageBox,
    QTextEdit, QSplitter, QFrame, QProgressBar, QComboBox,
...
class TaskExecutionHistoryDialog(QDialog):
    ...
    def _create_execution_list_frame(self) -> QFrame:
        ...
        # 连接信号
        self.execution_table.itemSelectionChanged.connect(self._on_execution_selection_changed)
    ...
    def _load_execution_history(self):
        ...
        try:
            if self.task_id:
                # 加载特定任务的执行历史
                self.execution_records = self.task_manager.get_task_executions(self.task_id)
            else:
                # 加载所有任务的执行历史
    ...
    def _populate_execution_table(self):
        ...
        for row, record in enumerate(self.execution_records):
            ...
            self.execution_table.setItem(row, 0, QTableWidgetItem(execution_id[:8] + '...' if len(execution_id) > 8 else execution_id))
    ...
    def _show_execution_details(self, record: Dict[str, Any]):
        ...
        start_time = record.get('start_time')
        if start_time:
            details.append(f"<b>开始时间:</b> {start_time.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        if error_message:
            details.append(f"<b>错误信息:</b> <span style='color: red;'>{error_message}</span>")
            
        # 执行日志
        execution_log = record.get('execution_log')
        if execution_log:
            details.append(f"<b>执行日志:</b>")
            details.append(f"<pre style='background-color: #f5f5f5; padding: 10px; border-radius: 5px;'>{execution_log}</pre>")
