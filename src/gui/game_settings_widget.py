...
class GameSettingsWidget(SettingsWidget):
    """游戏设置界面组件"""
    ...
    def _create_button_layout(self) -> QHBoxLayout:
        """创建按钮布局"""
        buttons = [
            {'text': '测试游戏连接', 'callback': self._test_game_connection, 'width': 120},
    ...
    def _load_settings(self):
        ...
        try:
            ...
            # 加载其他设置
            self.detection_timeout_spin.setValue(
                game_config.get("detection_timeout", 30)
            )
            self.detection_interval_spin.setValue(
                game_config.get("detection_interval", 500)
            )
            self.enable_detection_check.setChecked(
                game_config.get("enable_detection", True)
            )
        ...
        try:
            ...
            if detected_path:
                ...
                QMessageBox.information(
                    self, "检测成功", f"已检测到游戏路径：\n{detected_path}"
                )
            else:
                QMessageBox.information(
                    self, "检测失败", "未能自动检测到游戏路径，请手动选择。"
                )
