...
class AutomationSettingsWidget(SettingsWidget):
    """自动化配置界面组件"""
    ...
    def _create_operation_tab(self) -> QWidget:
        ...
        # 基础操作设置组
        basic_group = self.create_group_box("基础操作")
        ...
        # 检测设置组
        detection_group = self.create_group_box("检测设置")
        detection_layout = self.create_form_layout()
    ...
    def _create_button_layout(self) -> QHBoxLayout:
        """创建按钮布局"""
        buttons_config = [
            {"text": "测试自动化", "callback": self._test_automation_settings, "width": 100},
            {"text": "导入配置", "callback": self._import_settings, "width": 80},
            {"text": "导出配置", "callback": self._export_settings, "width": 80},
            {"text": "重置为默认", "callback": self._reset_to_defaults, "width": 100},
            {"text": "应用设置", "callback": self._apply_settings, "width": 80}
        ]
        return self.create_button_layout(buttons_config)
        ...
        for key, widget in widgets_mapping.items():
            if hasattr(widget, "valueChanged"):
                widget.valueChanged.connect(
                    lambda value, k=key: self._on_setting_changed(
                        "automation", k, value
                    )
                )
            elif hasattr(widget, "toggled"):
                widget.toggled.connect(
                    lambda checked, k=key: self._on_setting_changed(
                        "automation", k, checked
                    )
                )
    ...
    def _load_settings(self):
        ...
        try:
            ...
            # 加载操作参数
            self.click_delay_spin.setValue(
                int(automation_config.get("click_delay", 200))
            )
            self.key_delay_spin.setValue(int(automation_config.get("key_delay", 100)))
            self.operation_interval_spin.setValue(
                int(automation_config.get("operation_interval", 500))
            )
            self.random_delay_check.setChecked(
                automation_config.get("random_delay_enabled", True)
            )
            self.random_min_spin.setValue(
                int(automation_config.get("random_delay_min", 50))
            )
            self.random_max_spin.setValue(
                int(automation_config.get("random_delay_max", 200))
            )
            self.detection_accuracy_slider.setValue(
                int(automation_config.get("detection_accuracy", 85))
            )
            self.retry_count_spin.setValue(int(automation_config.get("retry_count", 3)))
            self.timeout_spin.setValue(
                int(automation_config.get("operation_timeout", 30))
            )
            ...
            self.exception_check.setChecked(
                automation_config.get("exception_check", True)
            )
            self.auto_recovery_check.setChecked(
                automation_config.get("auto_recovery", True)
            )
            self.max_runtime_spin.setValue(
                int(automation_config.get("max_runtime", 120))
            )
            self.cpu_limit_slider.setValue(int(automation_config.get("cpu_limit", 50)))
            self.memory_limit_spin.setValue(
                int(automation_config.get("memory_limit", 512))
            )
            self.emergency_key_edit.setText(
                automation_config.get("emergency_key", "F12")
            )
            self.mouse_corner_check.setChecked(
                automation_config.get("mouse_corner_stop", False)
            )
            ...
            self.thread_count_spin.setValue(
                int(automation_config.get("thread_count", 2))
            )
            self.concurrent_tasks_spin.setValue(
                int(automation_config.get("concurrent_tasks", 1))
            )
            self.image_cache_check.setChecked(
                automation_config.get("image_cache", True)
            )
            self.cache_size_spin.setValue(int(automation_config.get("cache_size", 100)))
            self.preload_check.setChecked(
                automation_config.get("preload_resources", False)
            )
            self.smart_wait_check.setChecked(automation_config.get("smart_wait", True))
            self.realtime_monitor_check.setChecked(
                automation_config.get("realtime_monitor", True)
            )
            self.monitor_interval_spin.setValue(
                int(automation_config.get("monitor_interval", 5))
            )
            self.performance_warning_check.setChecked(
                automation_config.get("performance_warning", True)
            )
            ...
            self.task_interval_spin.setValue(
                int(automation_config.get("task_interval", 10))
            )
    ...
    def _apply_settings(self):
        ...
        try:
            ...
            self.config_manager.set_setting(
                "automation", "task_priorities", task_priorities
            )
            ...
            try:
                ...
                QMessageBox.warning(
                    self, "应用失败", "自动化规则JSON格式错误，请检查后重试"
                )
                return
