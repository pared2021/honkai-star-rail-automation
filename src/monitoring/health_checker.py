...
class BaseHealthCheck:
    ...
    def get_status_from_value(self, value: float, metric_name: str) -> HealthStatus:
        """根据数值和阈值确定状态"""
        if self.config.critical_threshold is not None and value >= self.config.critical_threshold:
            return HealthStatus.CRITICAL
        elif self.config.warning_threshold is not None and value >= self.config.warning_threshold:
...
class FileSystemHealthCheck(BaseHealthCheck):
    ...
    def _perform_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            # 确定状态
            accessibility_rate = accessible_paths / total_paths if total_paths > 0 else 0
            ...
            return HealthCheckResult(
                ...
                metrics={
                    'accessibility_rate': accessibility_rate * 100
                }
            )
...
class ProcessHealthCheck(BaseHealthCheck):
    ...
    def _perform_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            # 确定状态
            running_rate = len(running_processes) / len(self.process_names) if self.process_names else 1.0
...
class ServiceHealthCheck(BaseHealthCheck):
    ...
    def __init__(self, config: HealthCheckConfig, service_checker: Callable[[], bool], 
...
class HealthChecker(QObject):
    ...
    def get_results_history(self, name: str, limit: int = None) -> List[HealthCheckResult]:
        """获取检查结果历史"""
        results = self.check_results.get(name, [])
        if limit:
    ...
    def _setup_default_checks(self):
        ...
        self.add_health_check("system_resources", SystemResourceHealthCheck(system_config))
        ...
        if self.db_manager:
            ...
            self.add_health_check("database", DatabaseHealthCheck(db_config, self.db_manager))
        ...
        self.add_health_check("filesystem", FileSystemHealthCheck(fs_config, important_paths))
    ...
    def _start_health_check(self, name: str, health_check: BaseHealthCheck):
        ...
        logger.debug(f"启动健康检查: {name}, 间隔: {health_check.config.check_interval_seconds}秒")
    ...
    def _process_check_result(self, name: str, result: HealthCheckResult):
        ...
        previous_status = previous_result.status if previous_result else HealthStatus.UNKNOWN
        ...
        if len(self.check_results[name]) > self.max_history_size:
            self.check_results[name] = self.check_results[name][-self.max_history_size:]
        ...
        if result.status != previous_status:
            self.health_status_changed.emit(name, previous_status.value, result.status.value)
            
            # 发送事件
            self.event_bus.emit('health_status_changed', {
                'component': name,
            ...
            logger.info(f"组件 {name} 健康状态变化: {previous_status.value} -> {result.status.value}")
    ...
    def _update_statistics(self):
        """更新统计信息"""
        self.stats['total_checks'] = sum(len(results) for results in self.check_results.values())
        ...
        for name in self.health_checks:
            latest_result = self.get_latest_result(name)
            if latest_result:
                status_counts[latest_result.status] = status_counts.get(latest_result.status, 0) + 1
    ...
    def _update_overall_health_status(self):
        ...
        if new_overall_status != previous_overall_status:
            ...
            # 发送事件
            self.event_bus.emit('overall_health_changed', {
                'previous_status': previous_overall_status.value,
                'current_status': new_overall_status.value,
                'timestamp': datetime.now().isoformat()
            })
            
            logger.info(f"整体健康状态变化: {previous_overall_status.value} -> {new_overall_status.value}")
