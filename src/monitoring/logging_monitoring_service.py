...
class LoggingMonitoringService(QObject):
    ...
    def add_log_entry(self, level: LogLevel, message: str, 
    ...
    def add_monitoring_event(self, event_type: MonitoringEventType,
    ...
    def get_logs(self, level: Optional[LogLevel] = None,
        ...
        if task_id:
            filtered_logs = [entry for entry in filtered_logs if entry.task_id == task_id]
        
        return filtered_logs
    
    def get_events(self, event_type: Optional[MonitoringEventType] = None,
                  source: Optional[str] = None,
                  hours: int = 24) -> List[MonitoringEvent]:
        """获取监控事件
        ...
        if source:
            filtered_events = [event for event in filtered_events if source in event.source]
    ...
    def add_alert_rule(self, name: str, condition: Callable[[Any], bool],
    ...
    def export_logs(self, filepath: str, hours: int = 24,
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        log_stats = {
            ...
            'warning': len([log for log in recent_logs if log.level == LogLevel.WARNING]),
        ...
        event_stats = {
            ...
            'critical': len([event for event in recent_events if event.severity == 'critical']),
            'warning': len([event for event in recent_events if event.severity == 'warning']),
        ...
        task_stats = {
            ...
            'running': len([status for status in all_task_status.values() 
                          if status.status == TaskStatus.RUNNING]),
            'completed': len([status for status in all_task_status.values() 
                           if status.status == TaskStatus.COMPLETED]),
            'failed': len([status for status in all_task_status.values() 
    ...
    def _setup_task_callbacks(self):
        ...
        self.task_monitor.task_completed.connect(
            lambda task_id, success, message: self.add_monitoring_event(
                MonitoringEventType.TASK_COMPLETED if success else MonitoringEventType.TASK_FAILED,
                "task_monitor",
    ...
    def _setup_default_alert_rules(self):
        ...
        self.add_alert_rule(
            "high_cpu_usage",
            lambda metrics: hasattr(metrics, 'cpu_percent') and metrics.cpu_percent > 80,
            lambda name, data: self.alert_triggered.emit("warning", "CPU使用率过高", data),
            "CPU使用率超过80%时触发"
        )
        ...
        self.add_alert_rule(
            "high_memory_usage",
            lambda metrics: hasattr(metrics, 'memory_percent') and metrics.memory_percent > 85,
            lambda name, data: self.alert_triggered.emit("critical", "内存使用率过高", data),
            "内存使用率超过85%时触发"
        )
        ...
        self.add_alert_rule(
            "task_failure",
            lambda event: (hasattr(event, 'event_type') and 
                          event.event_type == MonitoringEventType.TASK_FAILED),
    ...
    def _collect_system_metrics(self):
        ...
        try:
            ...
            # 获取任务统计
            all_task_status = self.task_monitor.get_all_task_status()
            active_tasks = len([s for s in all_task_status.values() 
                              if s.status == TaskStatus.RUNNING])
            completed_tasks = len([s for s in all_task_status.values() 
                                 if s.status == TaskStatus.COMPLETED])
            failed_tasks = len([s for s in all_task_status.values() 
                              if s.status == TaskStatus.FAILED])
            
            # 获取性能等级
            performance_level = self.performance_monitor._analyze_performance(current_metrics)
    ...
    def _get_system_health_status(self) -> Dict[str, Any]:
        ...
        all_task_status = self.task_monitor.get_all_task_status()
        if all_task_status:
            failed_count = len([s for s in all_task_status.values() 
                              if s.status == TaskStatus.FAILED])
    ...
    def _write_log_to_file(self, entry: LogEntry):
        """写入日志到文件"""
        try:
            log_file = self.log_directory / f"app_{datetime.now().strftime('%Y%m%d')}.log"
    ...
    def _save_logs_to_file(self):
        ...
        try:
            ...
            log_file = self.log_directory / f"logs_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _save_events_to_file(self):
        ...
        try:
            ...
            events_file = self.log_directory / f"events_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _save_metrics_to_file(self):
        ...
        try:
            ...
            metrics_file = self.log_directory / f"metrics_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            
            export_data = {
                'export_time': datetime.now().isoformat(),
                'metrics_count': len(self.system_metrics_history),
                'metrics': [metrics.to_dict() for metrics in self.system_metrics_history]
            }
