...
class TaskMonitor(QObject):
    ...
    def _monitor_task(self, task_id: str):
        ...
        try:
            ...
            # 检查自动化控制器状态
            if (self.automation_controller.current_task_id == task_id and 
                self.automation_controller.task_status != current_status):
                # 状态不同步，更新数据库
            ...
            update = TaskStatusUpdate(
                ...
                actions_completed=self.automation_controller.stats.get('actions_executed', 0),
                actions_failed=self.automation_controller.stats.get('actions_failed', 0)
            )
            
            # 检查状态是否有变化
            previous_update = self.task_status_cache.get(task_id)
            if (not previous_update or 
                previous_update.status != update.status or 
                abs(previous_update.progress - update.progress) > 0.01):
    ...
    def _apply_update(self, update: TaskStatusUpdate):
        ...
        if not previous_update or abs(previous_update.progress - update.progress) > 0.01:
        ...
        # 检查任务是否完成
        if update.status in [TaskStatus.COMPLETED, TaskStatus.FAILED, TaskStatus.STOPPED]:
            success = update.status == TaskStatus.COMPLETED
            self.task_completed.emit(update.task_id, success, update.message)
        
        logger.debug(f"任务状态更新: {update.task_id} -> {update.status.value} ({update.progress:.1%})")