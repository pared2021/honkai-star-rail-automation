...
from src.monitoring.config_manager import (
...
class MonitoringSystem:
    ...
    def __init__(self, event_bus=None, db_manager=None, config_directory: str = "./config"):
        """
        初始化监控系统
        ...
        # 初始化配置管理器
        self.config_manager = MonitoringConfigManager(config_directory)
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            # 发送系统启动事件
            if self.event_bus:
                self.event_bus.emit('monitoring_system_started', {
    ...
    def stop(self) -> bool:
        ...
        try:
            ...
            # 发送系统停止事件
            if self.event_bus:
                self.event_bus.emit('monitoring_system_stopped', {
                    'timestamp': self._get_current_timestamp()
                })
    ...
    def restart(self) -> bool:
        ...
        # 重新加载配置
        self.config_manager._load_configurations()
        self.config = self.config_manager.get_config()
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        if self.dashboard:
            status['components']['dashboard'] = {
                'available': True,
                'visible': self.dashboard.isVisible() if hasattr(self.dashboard, 'isVisible') else False
            }
    ...
    def export_monitoring_data(self, file_path: str, include_config: bool = True) -> bool:
        """导出监控数据"""
        try:
            export_data = {
            ...
            if include_config:
                export_data['configuration'] = self.config_manager._config_to_dict(self.config)
    ...
    def update_config(self, config_type: ConfigType, config_data: Dict[str, Any]) -> bool:
        """更新配置"""
        success = self.config_manager.update_config(config_type, config_data)
        if success:
            # 重新加载配置
            self.config = self.config_manager.get_config()
    ...
    def _setup_component_connections(self):
        ...
        if self.health_checker and self.alert_manager:
            ...
            self.health_checker.health_status_changed.connect(
                lambda component, old_status, new_status: 
                self._handle_health_status_change(component, old_status, new_status)
            )
    ...
    def _handle_health_status_change(self, component: str, old_status: str, new_status: str):
        """处理健康状态变化"""
        if self.alert_manager:
            # 根据健康状态变化创建相应的告警
            if new_status == 'critical':
                self.alert_manager.create_alert(
                    ...
                    details={'component': component, 'old_status': old_status, 'new_status': new_status}
                )
    ...
    def _notify_config_change(self, config_type: ConfigType, config_data: Dict[str, Any]):
        """通知组件配置变更"""
        # 根据配置类型通知相应组件
    ...
    def _on_config_changed(self, config_type: ConfigType, config_data: Dict[str, Any]):
        ...
        # 发送配置变更事件
        if self.event_bus:
            self.event_bus.emit('monitoring_config_changed', {
...
def initialize_monitoring_system(event_bus=None, db_manager=None, 
                               config_directory: str = "./config") -> MonitoringSystem:
    """初始化监控系统的便捷函数"""
