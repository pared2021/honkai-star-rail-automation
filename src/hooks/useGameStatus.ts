...
interface UseGameStatusReturn {
  ...
  checkAndPromptIfNeeded: (actionName?: string) => Promise<boolean>;
}

export const useGameStatus = (): UseGameStatusReturn => {
  const [gameStatus, setGameStatus] = useState<GameStatus | null>(null);
  const [loading, setLoading] = useState(false);
  const [showGameDialog, setShowGameDialog] = useState(false);

  const checkGameStatus = useCallback(async (): Promise<GameStatus> => {
    setLoading(true);
    try {
  ...
  const checkAndPromptIfNeeded = useCallback(async (actionName = '执行此操作'): Promise<boolean> => {
