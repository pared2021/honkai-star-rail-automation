...
class BaseAsyncService(ABC):
    ...
    async def _change_status(self, new_status: ServiceStatus):
        ...
        logger.debug(
            f"服务状态变更: {self.config.name} {old_status.value} -> {new_status.value}"
        )
            ...
            while self._status == ServiceStatus.RUNNING:
                try:
                    ...
                    if (
                        ...
                        logger.warning(
                            f"健康检查失败，尝试重启服务: {self.config.name}"
                        )
                        await self.restart()
    ...
    async def _acquire_operation_slot(self):
        ...
        if (
            self._metrics.current_concurrent_operations
            > self._metrics.max_concurrent_operations_reached
        ):
            self._metrics.max_concurrent_operations_reached = (
                self._metrics.current_concurrent_operations
            )
    ...
    async def execute_with_metrics(
        ...
        async with self._acquire_operation_slot():
            ...
            try:
                ...
                error = ServiceError(
                    f"操作失败: {operation_name}, {e}", original_error=e
                )
                await self._handle_error(error)
                raise error
...
class AsyncServiceManager:
    ...
    def register_service(
        self, service: BaseAsyncService, dependencies: Optional[List[str]] = None
    ):
        """
        注册服务
...
def register_global_service(
    service: BaseAsyncService, dependencies: Optional[List[str]] = None
):
    """
    注册全局服务
