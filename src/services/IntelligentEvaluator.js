...
class IntelligentEvaluator {
    constructor(dataCollector, dbService) {
        Object.defineProperty(this, "dataCollector", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "evaluationCache", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
    ...
    assessDataQuality(sessions) {
        ...
        factors += 0.3;
        // 数据完整性评分
        const completeSessionsCount = sessions.filter(s => s.endTime && s.totalDuration && s.metrics.length > 0).length;
        const completenessScore = completeSessionsCount / sessions.length;
        qualityScore += completenessScore * 0.4;
        factors += 0.4;
        // 数据新鲜度评分
        const now = Date.now();
        const recentSessions = sessions.filter(s => now - s.startTime.getTime() < 7 * 24 * 60 * 60 * 1000 // 7天内
        ).length;
     ...
     * 计算执行统计数据
     */
    calculateExecutionStatistics(sessions) {
        ...
        // 按时间段分析成功率
        const successRateByTimeOfDay = {};
        const successRateByDayOfWeek = {};
     ...
     * 分析瓶颈
     */
    async analyzeBottlenecks(strategy, sessions) {
        const bottlenecks = [];
        // 分析每个步骤的性能
        for (const step of strategy.steps) {
            const stepMetrics = sessions.flatMap(s => s.metrics.filter(m => m.stepId === step.id));
            if (stepMetrics.length === 0)
                continue;
            // 分析超时问题
            const timeouts = stepMetrics.filter(m => m.duration && m.duration > (step.timeout || 30000));
     ...
     * 生成优化建议
     */
    async generateOptimizationSuggestions(strategy, stats, bottlenecks) {
        const suggestions = [];
        // 基于瓶颈生成建议
    ...
    calculateEfficiency(stats, performance) {
        // 基于执行时间稳定性和资源使用效率
    ...
    calculateStability(sessions, performance) {
        // 基于系统稳定性和FPS稳定性
    ...
    calculateUserFriendliness(strategy, stats) {
        // 基于步骤复杂度和执行时间
    ...
    calculateResourceConsumption(performance) {
        // 资源消耗越低评分越高
    ...
    // 辅助方法
    calculateNetworkStability(history) {
        if (history.length === 0)
            return 1;
    ...
    calculateSystemStability(history) {
        if (history.length === 0)
            return 1;
        // 基于FPS和CPU使用率的稳定性
    ...
    identifyStrengths(scores, stats) {
        const strengths = [];
    ...
    identifyWeaknesses(scores, bottlenecks) {
        const weaknesses = [];
    ...
    assessRiskFactors(sessions, performance) {
        const risks = [];
        // 系统性能风险
        if (performance.averageCpuUsage > 80) {
            risks.push({
        ...
        // 游戏版本风险
        const recentFailures = sessions.filter(s => s.status === 'failed' &&
            Date.now() - s.startTime.getTime() < 24 * 60 * 60 * 1000).length;
    ...
    // 数据库操作
    async getExecutionHistory(_strategyId) {
     ...
     * 获取评估历史
     */
    async getEvaluationHistory(_strategyId) {
