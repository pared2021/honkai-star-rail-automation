import { Strategy, StrategyStep, StrategyEvaluation, StrategyAnalysisResult, AnalysisFactor, TaskInfo, TaskType, AnalysisPatterns, ScoreDetails, TestResults, SuccessPattern } from '../types/index.js';
...
export class StrategyAnalyzer {
  ...
  private analysisQueue: Map<string, Strategy[]> = new Map();
   ...
   * 开始分析任务攻略
   */
  public async startAnalysis(taskId: string, maxIterations: number = 5): Promise<StrategyAnalysisResult> {
   ...
   * 生成新的攻略方案
   */
  private async generateStrategy(taskInfo: TaskInfo, existingStrategies: Strategy[]): Promise<Omit<Strategy, 'id' | 'createdAt' | 'lastUpdated'>> {
    // 分析现有攻略的特点
    ...
    const strategy: Omit<Strategy, 'id' | 'createdAt' | 'lastUpdated'> = {
      taskInfoId: taskInfo.id!,
   ...
   * 评估攻略方案
   */
  private async evaluateStrategies(strategies: Strategy[], taskInfo: TaskInfo): Promise<StrategyEvaluation[]> {
   ...
   * 评估单个攻略
   */
  private async evaluateStrategy(strategy: Strategy, taskInfo: TaskInfo): Promise<Omit<StrategyEvaluation, 'id' | 'createdAt'>> {
    // 计算各项评分
    ...
    const evaluation: Omit<StrategyEvaluation, 'id'> = {
   ...
   * 计算时间效率得分
   */
  private calculateTimeScore(strategy: Strategy, taskInfo: TaskInfo): number {
    const estimatedTime = strategy.estimatedTime;
    const baseTime = taskInfo.estimatedTime || 60;
    
    if (estimatedTime <= baseTime * 0.8) return 100;
    if (estimatedTime <= baseTime) return 85;
    if (estimatedTime <= baseTime * 1.2) return 70;
    if (estimatedTime <= baseTime * 1.5) return 50;
    return 30;
  }
   ...
   * 计算难度得分
   */
  private calculateDifficultyScore(strategy: Strategy): number {
    ...
    if (difficultyLevel <= 2) return 100;
    if (difficultyLevel <= 3) return 80;
    if (difficultyLevel <= 4) return 60;
    return 40;
  }
   ...
   * 计算成功率得分
   */
  private calculateSuccessScore(strategy: Strategy): number {
   ...
   * 计算可靠性得分
   */
  private calculateReliabilityScore(strategy: Strategy): number {
    // 基于步骤数量和复杂度计算
    const stepCount = strategy.steps.length;
    const complexSteps = strategy.steps.filter(step => 
   ...
   * 计算资源消耗得分
   */
  private calculateResourceScore(strategy: Strategy): number {
    // 基于所需资源计算
    const resourceCount = strategy.requirements?.length || 0;
    
    if (resourceCount === 0) return 100;
    if (resourceCount <= 2) return 85;
    if (resourceCount <= 4) return 70;
    return 50;
  }
   ...
   * 找到最优攻略
   */
  private findBestStrategy(strategies: Strategy[], evaluations: StrategyEvaluation[]): Strategy {
   ...
   * 获取分析因子
   */
  private getAnalysisFactors(): AnalysisFactor[] {
    return [
      {
        name: '时间效率',
        weight: 0.25,
   ...
   * 生成推荐建议
   */
  private generateRecommendations(bestStrategy: Strategy, evaluations: StrategyEvaluation[]): string[] {
    const recommendations: string[] = [];
    
    const bestEvaluation = evaluations.find(e => e.strategyId === bestStrategy.id);
    if (!bestEvaluation) return recommendations;
  ...
  private calculateAverageTime(_strategies: Strategy[]): number {
  ...
  private generateStrategyDescription(taskInfo: TaskInfo, steps: StrategyStep[]): string {
    return `针对${taskInfo.taskName}的优化攻略，包含${steps.length}个步骤，预计用时${this.calculateEstimatedTime(steps)}分钟`;
  }

  private calculateEstimatedTime(steps: StrategyStep[]): number {
    return steps.reduce((total, step) => total + (step.timeout || 30), 0);
  }

  private calculateDifficulty(steps: StrategyStep[]): number {
  ...
  private estimateSuccessRate(steps: StrategyStep[], taskInfo: TaskInfo): number {
    // 基于步骤复杂度和任务难度估算成功率
    const baseRate = 95;
  ...
  private extractRequirements(steps: StrategyStep[]): string[] {
  ...
  private generateTips(taskInfo: TaskInfo, steps: StrategyStep[]): string[] {
    const tips: string[] = [];
  ...
  private generateStrategyTags(taskInfo: TaskInfo, steps: StrategyStep[]): string[] {
    const tags: string[] = [taskInfo.taskType];
  ...
  private generateFeedback(strategy: Strategy, scores: ScoreDetails): string {
  ...
  private generateTestResults(_strategy: Strategy): TestResults {
    return {
  ...
  private generateStrategyRecommendations(_strategy: Strategy): string[] {
    const recommendations: string[] = [];
