...
export class GameMonitor {
  ...
  // 已知的第三方工具列表
  private readonly knownThirdPartyTools: ThirdPartyToolInfo[] = [
    {
      name: '游戏加加',
   ...
   * 开始监控
   */
  public startMonitoring(options?: { interval?: number }): void {
    ...
    // 启动滤镜监控（较低频率）
    this.filterMonitorInterval = this.filterDetector.startFilterMonitoring(
      (report) => {
   ...
   * 执行监控检查
   */
  private async performMonitorCheck(): Promise<void> {
    try {
   ...
   * 检查状态变化并触发事件
   */
  private checkStatusChanges(currentStatus: GameMonitorStatus): void {
    if (!this.lastStatus) return;

    // 检查游戏启动/停止
    if (currentStatus.isGameRunning !== this.lastStatus.isGameRunning) {
      const eventType: GameEventType = currentStatus.isGameRunning ? 'game_started' : 'game_stopped';
   ...
   * 检查窗口是否发生变化
   */
  private hasWindowChanged(current: GameWindowInfo | null, previous: GameWindowInfo | null): boolean {
   ...
   * 获取新检测到的第三方工具
   */
  private getNewlyDetectedTools(current: ThirdPartyToolInfo[], previous: ThirdPartyToolInfo[]): ThirdPartyToolInfo[] {
   ...
   * 获取当前游戏状态
   */
  public async getGameStatus(): Promise<GameMonitorStatus> {
   ...
   * 获取游戏进程信息
   */
  public async getGameProcessInfo(): Promise<GameProcessInfo | null> {
    try {
      // 在实际实现中，这里会调用系统API来获取进程信息
      // 这里提供一个模拟实现
      const processes = await this.getRunningProcesses();
      const gameProcess = processes.find(p => 
        p.processName.toLowerCase().includes('starrail') ||
   ...
   * 检测第三方工具
   */
  public async detectThirdPartyTools(): Promise<ThirdPartyToolInfo[]> {
   ...
   * 生成滤镜检测报告
   */
  public async generateFilterReport() {
   ...
   * 检测游戏进程是否被注入
   */
  public async detectGameInjection(gameProcessId: number) {
   ...
   * 检测驱动级滤镜
   */
  public async detectDriverLevelFilters() {
   ...
   * 检查游戏是否响应
   */
  private async checkGameResponding(processId: number): Promise<boolean> {
    try {
      // 在实际实现中，这里会检查进程是否响应
      // 这里提供一个模拟实现
      return true;
   ...
   * 获取运行中的进程列表（模拟实现）
   */
  private async getRunningProcesses(): Promise<GameProcessInfo[]> {
    // 在实际实现中，这里会调用系统API
    // 这里返回模拟数据
    return [
      {
   ...
   * 获取活动窗口列表（模拟实现）
   */
  private async getActiveWindows(): Promise<GameWindowInfo[]> {
    // 在实际实现中，这里会调用系统API
    // 这里返回模拟数据
    return [
      {
   ...
   * 获取监控状态
   */
  public getStatus(): GameMonitorStatus {
    return {
      isRunning: this.monitorInterval !== null,
      isMonitoring: this.monitorInterval !== null,
      processInfo: null,
   ...
   * 获取监控统计信息
   */
  public getMonitorStats(): GameMonitorStats {
    return {
      totalChecks: 0,
      successfulChecks: 0,
      failedChecks: 0,
