...
from core.exception_recovery import (
    ExceptionRecovery, ExceptionType, RecoveryAction, 
    ExceptionInfo, RecoveryStrategy, RecoveryResult
)
...
class ErrorHandlingService(BaseAsyncService, QObject):
    """错误处理和恢复服务"""
    
    # 信号定义
    error_detected = pyqtSignal(object)  # ErrorContext
    error_handled = pyqtSignal(object)   # ErrorHandlingResult
    recovery_started = pyqtSignal(str, str)  # error_id, recovery_action
    recovery_completed = pyqtSignal(str, bool)  # error_id, success
    ...
    async def stop(self) -> bool:
        ...
        try:
            ...
            self.event_bus.unsubscribe("automation.error", self._handle_automation_error)
    ...
    async def handle_error(self, 
                          ...
                          retry_config: Optional[RetryConfig] = None) -> ErrorHandlingResult:
        """处理错误"""
        ...
        # 发送错误检测信号
        self.error_detected.emit(error_context)
        
        logger.warning(f"处理错误: {error_context.error_type} - {error_context.error_message}")
        ...
        try:
            ...
            # 发送处理完成信号
            self.error_handled.emit(result)
    ...
    async def _execute_with_retry(self, 
                                 error_context: ErrorContext,
                                 config: RetryConfig) -> ErrorHandlingResult:
        """执行重试逻辑"""
        if config.policy == RetryPolicy.NONE:
            return ErrorHandlingResult(
                success=False,
                error_context=error_context,
                final_status="no_retry"
            )
    ...
    async def _attempt_recovery(self, error_context: ErrorContext) -> Optional[RecoveryResult]:
        ...
        try:
            ...
            exception_info = ExceptionInfo(
                exception_type=self._map_error_to_exception_type(error_context.error_type),
    ...
    def _on_recovery_started(self, strategy, exception_info: ExceptionInfo):
        ...
        self.recovery_started.emit(error_id, action)
    ...
    def _on_recovery_completed(self, result: RecoveryResult):
        ...
        self.recovery_completed.emit(error_id, result.success)
