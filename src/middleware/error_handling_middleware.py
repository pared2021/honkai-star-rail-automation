...
from application.error_handling_service import (
    ErrorContext,
    ErrorHandlingService,
    ErrorSeverity,
)
from config.error_recovery_strategies import ErrorCategory, error_recovery_config
...
class ErrorHandlingMiddleware:
    ...
    def _create_error_context(
        self, error: Exception, context: Optional[Dict[str, Any]], source: Optional[str]
    ) -> ErrorContext:
        """创建错误上下文"""
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                try:
                    ...
                    handled = middleware.handle_error(
                        e, context, source or func.__name__
                    )
                    if not handled:
                        raise
                    return None
