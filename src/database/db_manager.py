...
class DatabaseManager:
    ...
    def add_execution_log(
        self, task_id: str, level: str, message: str, details: str = None
    ):
        """添加执行日志
    ...
    def cleanup_old_logs(self, days: int = 30):
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "DELETE FROM execution_logs WHERE timestamp < datetime('now', '-{} days')".format(
                    days
                )
            )
    ...
    def get_task_templates(
        self, template_type: str = None, is_public: bool = None
    ) -> List[Dict[str, Any]]:
        """获取任务模板列表
