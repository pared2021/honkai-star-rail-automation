...
from adapters.sync_adapter import async_to_sync, get_global_sync_adapter
...
class TaskManagerAdapter:
    ...
    def create_task_sync(
        self, config: Union[TaskConfig, Dict[str, Any]], user_id: str = "default"
    ) -> str:
        """
        同步创建任务（兼容旧接口）
    ...
    def resume_task(self, task_id: str) -> bool:
        ...
        try:
            # 更新任务状态为运行中并重新执行
            self.sync_adapter.run_async_blocking(
                self.task_manager.update_task_async(
                    task_id, "default", status="running"
                )
            )
            self.sync_adapter.run_async_blocking(
                self.task_manager.execute_task_async(task_id, "default")
    ...
    def _convert_dict_to_task_config(self, config_dict: Dict[str, Any]) -> TaskConfig:
        ...
        # 提取其他配置
        max_retry_count = config_dict.get(
            "max_retry_count", config_dict.get("retry_count", 3)
        )
        timeout_seconds = config_dict.get(
            "timeout_seconds", config_dict.get("max_duration", 300)
        )
    ...
    def _convert_dict_to_task_updates(
        self, updates_dict: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        将更新字典转换为标准格式
    ...
    def validate_task_config(
        self, config: Union[TaskConfig, Dict[str, Any]]
    ) -> tuple[bool, str]:
        """
        验证任务配置（兼容旧接口）
