...
class ModelAdapter:
    ...
    def dataclass_to_pydantic(dataclass_obj, target_class=None):
        ...
        try:
            ...
            raise ValueError(
                f"Cannot convert dataclass to {target_class.__name__}: {e}"
            )
        ...
        for obj in obj_list:
            try:
                if isinstance(obj, dict):
                    ...
                    raise ValueError(
                        f"Cannot convert {type(obj)} to {target_class.__name__}"
                    )
    ...
    def dict_to_task(data: Dict[str, Any]) -> Task:
        ...
        try:
            ...
            if "config" in normalized_data and isinstance(
                normalized_data["config"], dict
            ):
                normalized_data["config"] = ModelAdapter.dict_to_task_config(
        ...
        if "task_type" in normalized:
            ...
            if hasattr(task_type, "value"):
                ...
                for enum_item in TaskType:
                    if (
                        task_type == enum_item.value
                        or task_type.upper() == enum_item.name
                    ):
    ...
    def validate_conversion(
        original_data: Dict[str, Any], converted_obj: Union[TaskConfig, Task]
    ) -> bool:
        """验证转换是否正确"""
        try:
            # 将转换后的对象转回字典进行比较
            ...
            key_fields = (
                ["name", "task_type"]
                if isinstance(converted_obj, TaskConfig)
                else ["task_id", "user_id"]
            )
            ...
            for field in key_fields:
                if field in original_data:
                    ...
                    if original_value != converted_value:
                        logger.warning(
                            f"Field {field} mismatch: {original_value} != {converted_value}"
                        )
                        return False
