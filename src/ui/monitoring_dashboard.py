...
class LogViewer(QWidget):
    ...
    def add_log_entry(self, log_data: Dict[str, Any]):
        ...
        if self.current_module and self.current_module not in log_data.get(
            "module", ""
        ):
            return
...
class MonitoringDashboard(QWidget):
    ...
    def __init__(
        self, logging_monitoring_service: LoggingMonitoringService, parent=None
    ):
        super().__init__(parent)
    ...
    def create_performance_tab(self) -> QWidget:
        ...
        self.automation_rate_metric = MetricsWidget(
            "自动化频率", "0", "/min", "#FF5722"
        )
    ...
    def setup_connections(self):
        ...
        self.logging_service.monitoring_event_triggered.connect(
            self.on_monitoring_event
        )
        self.logging_service.system_metrics_updated.connect(
            self.on_system_metrics_updated
        )
    ...
    def update_dashboard(self):
        ...
        try:
            ...
            self.critical_events_metric.update_value(
                str(events.get("critical", 0)), "个"
            )
            ...
            # 更新服务状态指示器
            self.performance_indicator.update_status(
                "running"
                if self.logging_service.performance_monitor.monitoring_active
                else "stopped"
            )
            self.task_monitor_indicator.update_status(
                "running"
                if self.logging_service.task_monitor.is_monitoring
                else "stopped"
            )
