...
from src.monitoring import get_monitoring_system, initialize_monitoring_system
...
class MainWindowMVP:
    ...
    def on_start_automation(self):
        ...
        try:
            ...
            if basic_tasks:
                self.view.append_log(f"可用任务: {', '.join([task.name for task in basic_tasks])}")
    ...
    def update_status(self):
        ...
        try:
            ...
            if self.is_automation_running:
                ...
                if hasattr(self.view, 'monitoring_widget'):
                    status_info = {
                        'game_running': game_status.get('is_running', False),
    ...
    def _check_game_detector_health(self):
        """检查游戏检测器健康状态."""
        from src.monitoring.health_checker import HealthCheckResult, HealthStatus
        ...
        try:
            ...
            if game_status and game_status.get('status') == 'running':
                return HealthCheckResult(
                    component="game_detector",
