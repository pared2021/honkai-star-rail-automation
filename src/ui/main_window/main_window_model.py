...
class MainWindowModel(BaseModel):
    ...
    def _setup_validators(self):
        """设置验证器"""
        # 应用状态验证
        self.add_validator('app_status', 
                          lambda x: x in ['idle', 'running', 'paused', 'error'])
        
        # 日志级别验证
        self.add_validator('log_level',
                          lambda x: x in ['DEBUG', 'INFO', 'WARNING', 'ERROR'])
        
        # 数值验证
        for field in ['total_tasks', 'pending_tasks', 'running_tasks', 
                     'completed_tasks', 'failed_tasks', 'error_count']:
    ...
    async def _load_task_statistics(self):
        ...
        try:
            ...
            self.set_data('pending_tasks', status_counts.get(TaskStatus.PENDING, 0), validate=False)
            self.set_data('running_tasks', status_counts.get(TaskStatus.RUNNING, 0), validate=False)
            self.set_data('completed_tasks', status_counts.get(TaskStatus.COMPLETED, 0), validate=False)
            self.set_data('failed_tasks', status_counts.get(TaskStatus.FAILED, 0), validate=False)
            ...
            self.set_data('daily_mission_count', type_counts.get(TaskType.DAILY_MISSION, 0), validate=False)
            self.set_data('weekly_mission_count', type_counts.get(TaskType.WEEKLY_MISSION, 0), validate=False)
            self.set_data('event_mission_count', type_counts.get(TaskType.DAILY_MISSIONS, 0), validate=False)
            self.set_data('custom_task_count', type_counts.get(TaskType.CUSTOM, 0), validate=False)
    ...
    async def _load_system_status(self):
        ...
        try:
            ...
            if game_detected:
                ...
                self.set_data('game_window_title', game_info.get('window_title', ''), validate=False)
    ...
    async def _load_configuration(self):
        ...
        try:
            ...
            self.set_data('automation_enabled', automation_config.get('enabled', False), validate=False)
            self.set_data('auto_start_enabled', automation_config.get('auto_start', False), validate=False)
            
            # 加载UI配置
            ui_config = self._config_manager.get_ui_config()
            self.set_data('notification_enabled', ui_config.get('notifications', True), validate=False)
    ...
    async def _save_configuration(self):
        ...
        try:
            ...
            # 保存UI配置
            ui_config = {
                'notifications': self.get_data('notification_enabled', True)
            }
            self._config_manager.update_ui_config(ui_config)
            
            # 保存日志配置
            log_config = {
                'level': self.get_data('log_level', 'INFO')
            }
