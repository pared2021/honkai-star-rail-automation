...
from src.repositories.base_repository import EntityNotFoundError, QueryExecutionError
...
class SQLiteTaskRepository(TaskRepository):
    ...
    async def create_task(self, task: Task) -> str:
        ...
        try:
            ...
            try:
                insert_sql = """
                INSERT INTO tasks (
                    id, user_id, name, description, task_type, priority, status,
                ...
                await conn.execute(
                    ...
                    (
                        ...
                        task.last_error,
                        json.dumps([], ensure_ascii=False),  # execution_log
                        json.dumps(task.config.get("tags", []), ensure_ascii=False),
                        1,  # version
                    ),
                )
            ...
            raise QueryExecutionError("INSERT INTO tasks", str(e))
    ...
    async def get_task(self, task_id: str) -> Optional[Task]:
        ...
        try:
            ...
            raise QueryExecutionError("SELECT FROM tasks", str(e))
    ...
    async def update_task(self, task: Task) -> bool:
        ...
        try:
            ...
            raise QueryExecutionError("UPDATE tasks", str(e))
    ...
    async def delete_task(self, task_id: str) -> bool:
        ...
        try:
            ...
            raise QueryExecutionError("DELETE FROM tasks", str(e))
        ...
        try:
            ...
            try:
                ...
                for row in rows:
                    ...
                    if tags:
                        task_tags = task.config.get("tags", [])
    ...
    async def get_tasks_by_schedule(
        self, current_time: datetime, user_id: Optional[str] = None
    ) -> List[Task]:
        """获取需要调度的任务"""
        # 获取启用了调度的任务
        ...
        for task in all_tasks:
            schedule_config = task.config.get("schedule_config", {})
            if schedule_config.get("enabled", False) and task.status in [
                TaskStatus.PENDING,
    ...
    def _should_schedule_task(self, task: Task, current_time: datetime) -> bool:
        ...
        # 检查调度日期
        schedule_days = schedule_config.get("schedule_days", [])
        if schedule_days:
            weekday_names = [
                "monday",
                "tuesday",
        ...
        # 检查重复间隔
        repeat_interval = schedule_config.get("repeat_interval", 0)
        if repeat_interval > 0 and task.last_executed_at:
            time_since_last = current_time - task.last_executed_at
            if time_since_last.total_seconds() < repeat_interval:
                return False

        # 检查最大执行次数
        max_executions = schedule_config.get("max_executions", 0)
        if max_executions > 0 and task.execution_count >= max_executions:
            return False
    ...
    async def record_task_execution(
        self, task_id: str, success: bool, message: Optional[str] = None
    ) -> bool:
        """记录任务执行结果"""
        task = await self.get_task(task_id)
        if not task:
    ...
    async def get_task_statistics(
        ...
        try:
            ...
            raise QueryExecutionError("Statistics query", str(e))
    ...
    async def search_tasks(
        ...
        try:
            ...
            raise QueryExecutionError("Search tasks", str(e))
    ...
    async def cleanup_old_tasks(
        ...
        try:
            ...
            raise QueryExecutionError("DELETE old tasks", str(e))
    ...
    def _row_to_task(self, row) -> Task:
        ...
        try:
            ...
            task = Task(
                task_id=row["id"],
                user_id=row["user_id"],
                config=config_data,  # 直接使用字典而不是TaskConfig对象
            ...
            raise RepositoryError(
                f"Failed to convert row to Task: {e}", "CONVERSION_ERROR", e
            )
