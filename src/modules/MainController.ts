...
export interface MainControllerConfig {
  ...
  gameDetectionInterval?: number; // 游戏检测间隔（毫秒）
  logLevel?: LogLevel;
  automation: {
    enableAutoScheduling: boolean;
    smartPauseResume: boolean;
...
export interface AutomationState {
  isAutoSchedulingActive: boolean;
  isPaused: boolean;
  pauseReason?: string;
...
interface GameMonitoringState {
  isMonitoring: boolean;
  lastGameStatus?: GameStatus;
  gameStatusHistory: GameStatusSnapshot[];
  windowStateChanges: WindowStateChange[];
...
interface WindowStateChange {
  timestamp: Date;
  previousState: 'active' | 'inactive' | 'minimized' | 'closed';
  currentState: 'active' | 'inactive' | 'minimized' | 'closed';
...
interface RecoveryStrategy {
  ...
  maxAttempts: number;
  cooldownMs: number;
  execute: (exception: ExceptionInfo) => Promise<boolean>;
}

interface ExceptionHandlingState {
  isEnabled: boolean;
...
interface OptimizationAction {
  id: string;
  type: 'task_throttling' | 'memory_cleanup' | 'cache_clear' | 'priority_adjustment' | 'resource_reallocation';
  description: string;
...
interface NotificationChannel {
  id: string;
  name: string;
  type: 'log' | 'email' | 'webhook' | 'file' | 'console';
  enabled: boolean;
...
export class MainController extends EventEmitter {
  ...
  private autoScheduleInterval?: NodeJS.Timeout;
  private pauseCheckInterval?: NodeJS.Timeout;
  
  // 游戏监控相关属性
  private gameMonitoringState: GameMonitoringState;
  private gameMonitorInterval?: NodeJS.Timeout;
  private windowStateCheckInterval?: NodeJS.Timeout;
  private gameRecoveryInterval?: NodeJS.Timeout;
  
  // 异常处理相关属性
  private exceptionHandlingState: ExceptionHandlingState;
  private recoveryCheckInterval?: NodeJS.Timeout;
  private systemStabilityCheckInterval?: NodeJS.Timeout;
  
  // 性能监控相关属性
  private performanceMonitoringState: PerformanceMonitoringState;
  private performanceMonitorInterval?: NodeJS.Timeout;
  private optimizationCheckInterval?: NodeJS.Timeout;
  ...
  // 事件系统相关属性
  private eventSystemState: EventSystemState;
  ...
  private notificationRateLimits: Map<string, RateLimit>;

  constructor(config: Partial<MainControllerConfig> = {}) {
    super();
  ...
  private setupModuleEventListeners(): void {
    ...
    this.taskExecutor.on('taskFailed', (task: Task, error: string) => {
      this.logEvent('error', `任务失败: ${task.taskType} (${task.id}) - ${error}`);
      this.handleTaskFailure(task, error);
      this.emit('taskFailed', task, error);
    });
    
    this.taskExecutor.on('taskStarted', (task: Task) => {
   ...
   * 尝试任务恢复
   */
  private async attemptTaskRecovery(task: Task): Promise<void> {
    try {
      // 检查游戏状态
   ...
   * 创建默认配置
   */
  private createDefaultConfiguration(): ConfigurationSchema {
    return {
      version: '1.0.0',
   ...
   * 更新配置
   */
  public async updateConfiguration(updates: Partial<MainControllerConfig>): Promise<void> {
    try {
      // 验证更新
   ...
   * 升级配置版本
   */
  public async upgradeConfiguration(targetVersion: string): Promise<void> {
    try {
   ...
   * 回滚配置版本
   */
  public async rollbackConfiguration(targetVersion: string): Promise<void> {
    try {
      // 查找目标版本的备份
      const targetBackup = this.configurationBackups.find(backup => 
        backup.config.version === targetVersion
      );
   ...
   * 恢复配置备份
   */
  public async restoreConfigurationBackup(backupId: string): Promise<void> {
    try {
   ...
   * 停止配置文件监听
   */
  private stopConfigurationWatcher(): void {
   ...
   * 停止配置管理
   */
  private stopConfigurationManagement(): void {
    this.stopAutoSave();
   ...
   * 停止任务
   */
  public async stopTask(taskId: string): Promise<ApiResponse<void>> {
    try {
      // 获取任务信息用于日志记录
   ...
   * 获取游戏状态
   */
  public async getGameStatus(): Promise<ApiResponse<GameStatus>> {
    try {
   ...
   * 获取事件日志
   */
  public getEventLogs(limit: number = 100): TaskLog[] {
   ...
   * 更新配置
   */
  public async updateConfig(newConfig: Partial<MainControllerConfig>): Promise<ApiResponse<void>> {
    try {
   ...
   * 获取当前配置
   */
  public getConfig(): MainControllerConfig {
    return { ...this.config };
  }
   ...
   * 停止自动调度
   */
  public stopAutoScheduling(): void {
   ...
   * 执行自动调度逻辑
   */
  private async performAutoScheduling(): Promise<void> {
    try {
   ...
   * 智能暂停恢复检查
   */
  private async checkSmartPauseResume(): Promise<void> {
    try {
   ...
   * 判断是否应该暂停自动化
   */
  private async shouldPauseAutomation(): Promise<{ reason: string } | null> {
    // 检查系统资源
   ...
   * 暂停自动化
   */
  private pauseAutomation(reason: string): void {
   ...
   * 计算最优任务槽位数
   */
  private calculateOptimalTaskSlots(availableSlots: number): number {
    // 基于系统性能调整
  ...
  private calculateTaskPriority(task: Task): TaskPriority {
    ...
    if (taskAge > 3600000) { // 超过1小时
      priority = Math.min(priority + 1, TaskPriority.URGENT) as TaskPriority;
    }
   ...
   * 获取自动化状态
   */
  public getAutomationState(): AutomationState {
   ...
   * 启动游戏状态监控
   */
  public startGameMonitoring(): void {
   ...
   * 停止游戏状态监控
   */
  public stopGameMonitoring(): void {
  ...
  private async checkGameStatus(): Promise<void> {
    try {
      ...
      // 记录状态快照
      const snapshot: GameStatusSnapshot = {
        timestamp: new Date(),
   ...
   * 处理游戏状态变化
   */
  private async handleGameStatusChange(previousStatus: GameStatus, currentStatus: GameStatus): Promise<void> {
    // 游戏启动
   ...
   * 确定窗口状态
   */
  private determineWindowState(isActive: boolean, windowRect?: { x: number; y: number; width: number; height: number }): 'active' | 'inactive' | 'minimized' | 'closed' {
    if (!windowRect) {
      return 'closed';
    }
   ...
   * 尝试游戏恢复
   */
  private async attemptGameRecovery(): Promise<void> {
   ...
   * 重启卡住的任务
   */
  private async restartStuckTask(task: Task): Promise<void> {
    try {
   ...
   * 游戏启动时恢复暂停的任务
   */
  private async resumePausedTasksOnGameStart(): Promise<void> {
   ...
   * 游戏关闭时暂停所有任务
   */
  private async pauseAllTasksOnGameClose(): Promise<void> {
    const runningTasks = this.taskExecutor.getRunningTasks();
   ...
   * 处理窗口失活
   */
  private async handleWindowDeactivation(): Promise<void> {
    // 检查失活时间
    const lastChange = this.gameMonitoringState.windowStateChanges[this.gameMonitoringState.windowStateChanges.length - 1];
   ...
   * 获取游戏监控状态
   */
  public getGameMonitoringState(): GameMonitoringState {
   ...
   * 获取游戏状态历史
   */
  public getGameStatusHistory(limit: number = 50): GameStatusSnapshot[] {
   ...
   * 获取窗口状态变化历史
   */
  public getWindowStateHistory(limit: number = 20): WindowStateChange[] {
  ...
  private initializeRecoveryStrategies(): void {
    ...
    this.exceptionHandlingState.recoveryStrategies.set('task_restart', {
      ...
      maxAttempts: 3,
      cooldownMs: 5000,
      execute: async (exception: ExceptionInfo) => {
   ...
   * 启动异常处理系统
   */
  public startExceptionHandling(): void {
   ...
   * 停止异常处理系统
   */
  public stopExceptionHandling(): void {
   ...
   * 获取最大恢复尝试次数
   */
  private getMaxRecoveryAttempts(severity: ExceptionInfo['severity']): number {
    switch (severity) {
      case 'low': return 2;
   ...
   * 尝试恢复异常
   */
  public async attemptRecovery(exception: ExceptionInfo): Promise<boolean> {
   ...
   * 选择恢复策略
   */
  private selectRecoveryStrategies(exception: ExceptionInfo): RecoveryStrategy[] {
    const strategies: RecoveryStrategy[] = [];
    
    switch (exception.type) {
      case 'task_error':
        strategies.push(
          this.exceptionHandlingState.recoveryStrategies.get('task_restart')!,
          this.exceptionHandlingState.recoveryStrategies.get('system_reset')!
        );
        break;
        
      case 'game_error':
        strategies.push(
          this.exceptionHandlingState.recoveryStrategies.get('game_reconnect')!,
          this.exceptionHandlingState.recoveryStrategies.get('system_reset')!
        );
        break;
      ...
      case 'system_error':
        strategies.push(
          this.exceptionHandlingState.recoveryStrategies.get('system_reset')!,
          this.exceptionHandlingState.recoveryStrategies.get('resource_cleanup')!
        );
        break;
   ...
   * 检查策略是否在冷却中
   */
  private async isStrategyCoolingDown(strategy: RecoveryStrategy): Promise<boolean> {
    // 这里可以实现更复杂的冷却逻辑
    // 简单实现：检查最后执行时间
  ...
  private async checkPendingRecoveries(): Promise<void> {
    const pendingExceptions = this.exceptionHandlingState.exceptions.filter(
      ex => !ex.isRecovered && ex.recoveryAttempts < ex.maxRecoveryAttempts
    );
  ...
  private checkSystemStability(): void {
    ...
    // 统计最近一小时的异常
    const recentExceptions = this.exceptionHandlingState.exceptions.filter(
      ex => ex.timestamp.getTime() > oneHourAgo
    );
    ...
    let newStability: ExceptionHandlingState['systemStability'] = 'stable';
   ...
   * 处理系统危险状态
   */
  private async handleCriticalSystemState(): Promise<void> {
   ...
   * 执行任务重启策略
   */
  private async executeTaskRestartStrategy(exception: ExceptionInfo): Promise<boolean> {
    try {
   ...
   * 执行游戏重连策略
   */
  private async executeGameReconnectStrategy(exception: ExceptionInfo): Promise<boolean> {
    try {
   ...
   * 执行系统重置策略
   */
  private async executeSystemResetStrategy(exception: ExceptionInfo): Promise<boolean> {
    try {
   ...
   * 执行资源清理策略
   */
  private async executeResourceCleanupStrategy(exception: ExceptionInfo): Promise<boolean> {
    try {
   ...
   * 获取异常处理状态
   */
  public getExceptionHandlingState(): ExceptionHandlingState {
   ...
   * 清理已恢复的异常
   */
  public clearRecoveredExceptions(): number {
    const beforeCount = this.exceptionHandlingState.exceptions.length;
    this.exceptionHandlingState.exceptions = this.exceptionHandlingState.exceptions.filter(
      ex => !ex.isRecovered
    );
   ...
   * 初始化优化操作
   */
  private initializeOptimizationActions(): void {
  ...
  private async checkOptimizationNeeds(): Promise<void> {
    ...
    try {
      const criticalAlerts = this.performanceMonitoringState.alerts.filter(
   ...
   * 执行自动优化
   */
  private async executeAutoOptimization(alerts: PerformanceAlert[]): Promise<void> {
    const now = new Date();
      ...
      switch (alert.metric) {
        case 'cpu':
          optimizationActions.push(
            ...this.performanceMonitoringState.optimizationActions.filter(
              action => action.type === 'task_throttling' || action.type === 'resource_reallocation'
            )
          );
          break;
        case 'memory':
          optimizationActions.push(
            ...this.performanceMonitoringState.optimizationActions.filter(
              action => action.type === 'memory_cleanup'
            )
          );
          break;
        case 'responseTime':
          optimizationActions.push(
            ...this.performanceMonitoringState.optimizationActions.filter(
              action => action.type === 'priority_adjustment'
            )
          );
          break;
      }
  ...
  private createPerformanceAlert(
    ...
    // 检查是否已存在相同的未解决告警
    const existingAlert = this.performanceMonitoringState.alerts.find(
      alert => alert.metric === metric && alert.type === type && !alert.isResolved
    );
   ...
   * 获取性能监控状态
   */
  public getPerformanceMonitoringState(): PerformanceMonitoringState {
   ...
   * 获取性能快照历史
   */
  public getPerformanceSnapshots(limit?: number): PerformanceSnapshot[] {
    const snapshots = this.performanceMonitoringState.snapshots;
   ...
   * 获取性能告警
   */
  public getPerformanceAlerts(onlyUnresolved = false): PerformanceAlert[] {
   ...
   * 紧急停止所有任务
   */
  public async emergencyStop(): Promise<ApiResponse<void>> {
    try {
  ...
  private initializeDefaultNotificationChannels(): void {
    ...
    // 文件日志通道
    const fileChannel: NotificationChannel = {
      id: 'file',
      name: '文件日志',
      type: 'file',
      enabled: true,
   ...
   * 检查频率限制
   */
  private checkRateLimit(channelId: string, rateLimit: RateLimit): boolean {
    const now = new Date();
  ...
  private async sendNotificationToChannel(eventOrChannelId: SystemEvent | string, channelOrMessage: NotificationChannel | string, eventData?: any): Promise<void> {
    ...
    // 原有的实现
    const event = eventOrChannelId as SystemEvent;
   ...
   * 写入文件通知
   */
  private async writeNotificationToFile(event: SystemEvent, channel: NotificationChannel, message: string): Promise<void> {
   ...
   * 发送邮件通知
   */
  private async sendEmailNotification(event: SystemEvent, channel: NotificationChannel, message: string): Promise<void> {
    try {
   ...
   * 发送Webhook通知
   */
  private async sendWebhookNotification(event: SystemEvent, channel: NotificationChannel, message: string): Promise<void> {
    try {
      const config = channel.config;
   ...
   * 获取事件系统状态
   */
  public getEventSystemState(): EventSystemState {
   ...
   * 持久化事件历史
   */
  public async persistEventHistory(): Promise<void> {
    try {
   ...
   * 加载事件历史
   */
  public async loadEventHistory(): Promise<void> {
    try {
      const eventHistoryPath = path.join(process.cwd(), 'data', 'event-history.json');
   ...
   * 导出事件历史
   */
  public async exportEventHistory(filePath: string, filter?: {
    eventType?: string;
    source?: string;
    severity?: string;
    startTime?: Date;
   ...
   * 导入事件历史
   */
  public async importEventHistory(filePath: string, mergeMode: 'replace' | 'merge' = 'merge'): Promise<void> {
    try {
      // 检查文件是否存在
   ...
   * 获取事件统计信息
   */
  public getEventStatistics(timeRange?: { startTime: Date; endTime: Date }): {
    totalEvents: number;
   ...
   * 动态添加事件监听器
   */
  public addEventListenerDynamic(
    id: string,
    eventType: string,
    handler: (event: SystemEvent) => void | Promise<void>,
    options?: {
   ...
   * 获取事件监听器列表
   */
  public getEventListeners(filter?: {
    eventType?: string;
    isActive?: boolean;
    priority?: number;
  }): EventListener[] {
   ...
   * 批量管理事件监听器
   */
  public batchManageEventListeners(operations: Array<{
        ...
        switch (operation.action) {
          case 'add':
            if (operation.eventType && operation.handler) {
              this.addEventListenerDynamic(
                operation.listenerId,
                operation.eventType,
                operation.handler,
                operation.options
              );
   ...
   * 高级事件过滤器
   */
  public createAdvancedEventFilter(config: {
    types?: string[];
   ...
   * 移除事件路由
   */
  public removeEventRoute(routeId: string): boolean {
   ...
   * 启用/禁用事件路由
   */
  public toggleEventRoute(routeId: string, enabled: boolean): boolean {
   ...
   * 条件路由构建器
   */
  public createConditionalRoute(config: {
    name: string;
    conditions: {
