// 任务信息收集模块
import { ImageRecognition, RecognitionResult } from './ImageRecognition.js';
import { TaskInfo, TaskInfoCollectionConfig, TaskType, TaskStatus, TaskReward } from '../types';
import { DatabaseService } from '../services/DatabaseService';
import { v4 as uuidv4 } from 'uuid';
import * as path from 'path';

export interface TaskInfoCollectionResult {
  success: boolean;
  taskInfo?: TaskInfo;
  error?: string;
  confidence: number;
...
export interface TaskUIElement {
  name: string;
  templatePath: string;
...
export class TaskInfoCollector {
  ...
  private config: TaskInfoCollectionConfig;
  private logger: {
  ...
  // 任务界面UI元素模板
  private readonly taskUIElements: Record<TaskType, TaskUIElement[]> = {
    [TaskType.MAIN]: [
  ...
  constructor(
    ...
    config: TaskInfoCollectionConfig
  ) {
    ...
    this.logger = {
      info: (msg: string) => console.log(`[TaskInfoCollector] ${msg}`),
      error: (msg: string, error?: any) => console.error(`[TaskInfoCollector] ${msg}`, error),
      debug: (msg: string) => console.debug(`[TaskInfoCollector] ${msg}`)
    };
  }
   ...
   * 自动收集当前屏幕上的任务信息
   */
  public async collectTaskInfo(): Promise<TaskInfoCollectionResult> {
    try {
   ...
   * 检测当前任务类型
   */
  private async detectTaskType(): Promise<TaskType | null> {
    const taskTypes = Object.keys(this.taskUIElements) as TaskType[];
   ...
   * 根据任务类型收集具体信息
   */
  private async collectTaskInfoByType(taskType: TaskType): Promise<TaskInfo | null> {
    try {
      const taskInfo: TaskInfo = {
        ...
        rewards: [],
        estimatedTime: 0,
        difficulty: 'easy' as 'easy' | 'medium' | 'hard' | 'extreme',
        location: '',
        isRepeatable: false,
   ...
   * 提取任务名称
   */
  private async extractTaskName(taskType: TaskType): Promise<string> {
    try {
      // 根据任务类型确定名称区域
   ...
   * 提取任务描述
   */
  private async extractTaskDescription(taskType: TaskType): Promise<string> {
    try {
   ...
   * 提取任务奖励
   */
  private async extractTaskRewards(taskType: TaskType): Promise<TaskReward[]> {
    try {
      ...
      if (result.found && result.text) {
        ...
        return rewardLines.map((line, index) => ({
          type: 'exp' as const, // 默认类型，实际应根据文本内容判断
          name: line.trim(),
   ...
   * 提取任务要求
   */
  private async extractTaskRequirements(taskType: TaskType): Promise<string[]> {
    try {
   ...
   * 获取任务名称区域
   */
  private getTaskNameRegion(taskType: TaskType): { x: number; y: number; width: number; height: number } | null {
    // 根据任务类型返回不同的名称区域坐标
    // 这些坐标需要根据实际游戏界面调整
    switch (taskType) {
      case TaskType.MAIN:
   ...
   * 获取任务描述区域
   */
  private getTaskDescriptionRegion(taskType: TaskType): { x: number; y: number; width: number; height: number } | null {
    switch (taskType) {
      case TaskType.MAIN:
   ...
   * 获取任务奖励区域
   */
  private getTaskRewardsRegion(taskType: TaskType): { x: number; y: number; width: number; height: number } | null {
    switch (taskType) {
      case TaskType.MAIN:
      case TaskType.SIDE:
      case TaskType.EVENT:
   ...
   * 获取任务要求区域
   */
  private getTaskRequirementsRegion(taskType: TaskType): { x: number; y: number; width: number; height: number } | null {
    switch (taskType) {
      case TaskType.MAIN:
      case TaskType.SIDE:
   ...
   * 估算任务难度
   */
  private estimateTaskDifficulty(taskInfo: TaskInfo): 'easy' | 'medium' | 'hard' | 'extreme' {
    ...
    // 将数字转换为字符串字面量
    if (finalScore <= 1) return 'easy';
    if (finalScore <= 2) return 'medium';
    if (finalScore <= 4) return 'hard';
    return 'extreme';
  }
   ...
   * 估算任务时间（分钟）
   */
  private estimateTaskTime(taskInfo: TaskInfo): number {
   ...
   * 计算整体置信度
   */
  private calculateOverallConfidence(taskInfo: TaskInfo): number {
   ...
   * 保存任务信息到数据库
   */
  private async saveTaskInfo(taskInfo: TaskInfo): Promise<void> {
    try {
   ...
   * 更新配置
   */
  public updateConfig(newConfig: Partial<TaskInfoCollectionConfig>): void {
   ...
   * 获取当前配置
   */
  public getConfig(): TaskInfoCollectionConfig {
    return { ...this.config };
  }
}