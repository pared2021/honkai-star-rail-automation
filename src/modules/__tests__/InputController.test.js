...
describe('InputController', () => {
    ...
    describe('安全检查测试', () => {
        ...
        test('应该检查操作间隔', async () => {
            ...
            // 设置更长的最小间隔用于测试
            inputController.minActionInterval = 200;
            // 第一次点击
            await inputController.click(100, 200);
    ...
    describe('日志记录测试', () => {
        ...
        test('应该限制日志数量', async () => {
            // 设置较小的日志限制进行测试
            const originalMaxLogs = inputController.maxLogEntries;
            inputController.maxLogEntries = 5;
            // 执行6次操作
            for (let i = 0; i < 6; i++) {
            ...
            // 恢复原始设置
            inputController.maxLogEntries = originalMaxLogs;
        });
    });
    describe('错误处理测试', () => {
        beforeEach(() => {
