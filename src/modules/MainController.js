...
export class MainController extends EventEmitter {
     ...
     * 处理任务失败
     */
    async handleTaskFailure(task, _error) {
        this.errorCount++;
    ...
    validateConfiguration(config) {
        ...
        try {
            ...
            if (configObj.gameDetection) {
                const gameDetection = configObj.gameDetection;
            ...
            if (configObj.taskExecution) {
                const taskExecution = configObj.taskExecution;
            ...
            if (configObj.performanceMonitoring) {
                const performanceMonitoring = configObj.performanceMonitoring;
            ...
            if (configObj.security) {
                const security = configObj.security;
    ...
    async triggerEventSubscriptions(event) {
        const eventObj = event;
        const subscriptions = this.eventSubscriptions.get(eventObj.type) || [];
        const toRemove = [];
    ...
    async sendNotificationToChannel(eventOrChannelId, channelOrMessage, eventData) {
        ...
        if (typeof eventOrChannelId === 'string') {
            ...
            const systemEvent = {
                id: eventDataObj.data?.id || uuidv4(),
                type: eventDataObj.type || 'notification',
                source: eventDataObj.source || 'system',
                timestamp: eventDataObj.timestamp ? new Date(eventDataObj.timestamp) : new Date(),
                severity: eventDataObj.severity || 'low',
                category: 'notification',
                tags: [],
                data: message,
                metadata: eventDataObj.data || {}
            };
     ...
     * 格式化通知消息
     */
    formatNotificationMessage(event, _channel) {
        const eventObj = event;
        const timestamp = new Date(eventObj.timestamp).toISOString();
    ...
    async writeNotificationToFile(event, channel, message) {
        ...
        if (format === 'json') {
            ...
            content = `"${new Date(eventObj.timestamp).toISOString()}","${eventObj.source}","${eventObj.type}","${eventObj.severity}","${JSON.stringify(eventObj.data).replace(/"/g, '""')}"\n`;
        }
        else {
            content = message + '\n';
        }
    ...
    async sendEmailNotification(event, channel, message) {
        try {
          ...
          <div style="font-family: Arial, sans-serif;">
            ...
            <p><strong>时间:</strong> ${new Date(eventObj.timestamp).toLocaleString()}</p>
        ...
        try {
            ...
            const payload = {
                event: {
                    ...
                    timestamp: new Date(eventObj.timestamp).toISOString(),
    ...
    cleanupEventHistory() {
        ...
        this.eventHistory.events = this.eventHistory.events.filter(event => {
            const eventObj = event;
            return now.getTime() - new Date(eventObj.timestamp).getTime() < retentionMs;
        });
    }
    /**
     * 添加通知渠道
    ...
    getEventHistory(filter) {
        ...
        if (filter) {
            ...
            if (filter.startTime) {
                events = events.filter(e => {
                    const eventObj = e;
                    return new Date(eventObj.timestamp) >= filter.startTime;
                });
            }
            if (filter.endTime) {
                events = events.filter(e => {
                    const eventObj = e;
                    return new Date(eventObj.timestamp) <= filter.endTime;
                });
            }
            if (filter.limit) {
    ...
    addEventListenerDynamic(id, eventType, handler, options) {
        ...
        // 按优先级排序
        this.eventSystemState.listeners.sort((a, b) => Number(b.priority || 0) - Number(a.priority || 0));
    }
    /**
     * 移除事件监听器
     */
    removeEventListener(listenerId) {
        ...
        if (index !== -1) {
            ...
            this.logEvent('info', '事件监听器已移除', {
                listenerId,
                eventType: String(listener.eventType || '')
            });
            return true;
        }
        ...
        return {
            ...
            condition: (event) => {
                try {
                    ...
                    // 检查严重程度
                    if (config.severities && !config.severities.includes(String(eventObj.severity || ''))) {
                        return false;
                    }
                    // 检查事件源
        ...
        const condition = (event) => {
            ...
            if (config.conditions.severity) {
                const severities = Array.isArray(config.conditions.severity)
                    ...
                    : [config.conditions.severity];
                const eventObj = event;
                if (!severities.includes(String(eventObj.severity || ''))) {
                    return false;
                }
            }
            // 检查事件源
        ...
        if (config.actions.notify) {
            handlers.push(async (event) => {
                const message = config.actions.notify.template
                    ...
                    : `事件通知: ${String(event.type || '')} - ${String(event.message || '')}`;
