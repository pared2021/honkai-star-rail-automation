...
export class GameDetector extends EventEmitter {
    constructor(config) {
        ...
        // 实时监控相关属性
        Object.defineProperty(this, "realTimeMonitoringEnabled", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        Object.defineProperty(this, "lastGameState", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: {
        ...
        Object.defineProperty(this, "stateChangeCallbacks", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
     ...
     * 模糊匹配进程名
     */
    fuzzyMatchProcessName(processName, targetName) {
        // 移除常见的后缀和前缀
        ...
        return keywords.some(keyword => cleanProcess.includes(keyword) && cleanTarget.includes(keyword));
    }
    /**
     * 获取游戏窗口
     */
        ...
        try {
            ...
            for (const windowTitle of prioritizedTitles) {
                const gameWindow = windows.find(w => {
                    try {
                        if (!w || typeof w.getTitle !== 'function')
                            return false;
     ...
     * 检查窗口标题是否匹配游戏窗口
     */
    isGameWindowTitle(windowTitle, targetTitle) {
     ...
     * 检查游戏窗口是否在指定区域内
     */
    async isGameWindowInRegion(region) {
        try {
            ...
            // 检查窗口是否与指定区域有重叠
            const overlap = (bounds.x < region.x + region.width &&
     ...
     * 启用实时监控
     */
    enableRealTimeMonitoring(interval = 2000) {
     ...
     * 检查状态变化并触发事件
     */
    checkStateChanges(currentState) {
        // 游戏启动检测
     ...
     * 检查窗口位置是否发生变化
     */
    hasWindowPositionChanged(oldPos, newPos) {
        if (!oldPos && !newPos)
            return false;
        if (!oldPos || !newPos)
            return true;
     ...
     * 通知状态变化
     */
    notifyStateChange(eventType, state) {
