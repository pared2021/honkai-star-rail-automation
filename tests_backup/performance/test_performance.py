...
from src.core.task_executor import ExecutorStatus, TaskExecutionContext, TaskExecutor
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            "task_creation": {
                ...
                "avg": (
                    statistics.mean(self.task_creation_times)
                    if self.task_creation_times
                    else 0
                ),
                "median": (
                    statistics.median(self.task_creation_times)
                    if self.task_creation_times
                    else 0
                ),
            ...
            "task_execution": {
                ...
                "avg": (
                    statistics.mean(self.task_execution_times)
                    if self.task_execution_times
                    else 0
                ),
                "median": (
                    statistics.median(self.task_execution_times)
                    if self.task_execution_times
                    else 0
                ),
                "max": (
                    max(self.task_execution_times) if self.task_execution_times else 0
                ),
                "min": (
                    min(self.task_execution_times) if self.task_execution_times else 0
                ),
            },
            "database_operations": {
                "count": len(self.database_operation_times),
                "avg": (
                    statistics.mean(self.database_operation_times)
                    if self.database_operation_times
                    else 0
                ),
                "median": (
                    statistics.median(self.database_operation_times)
                    if self.database_operation_times
                    else 0
                ),
                "max": (
                    max(self.database_operation_times)
                    if self.database_operation_times
                    else 0
                ),
                "min": (
                    min(self.database_operation_times)
                    if self.database_operation_times
                    else 0
                ),
            },
            "errors": {
                ...
                "rate": (
                    self.error_counts / self.total_operations
...
class TestPerformance:
    ...
    async def setup_performance_system(self):
        ...
        try:
            ...
            task_service = TaskApplicationService(
                task_service=task_domain_service, event_bus=event_bus
            )
        ...
        for batch_start in range(0, num_tasks, batch_size):
            ...
            for i in range(batch_start, batch_end):
                ...
                task = await task_service.create_task(
                    user_id="perf_test_user", config=config
                )
                task_end = time.time()
            ...
            print(
                f"批次 {batch_start//batch_size + 1}: {batch_end - batch_start} 个任务, 耗时 {batch_time:.3f}s"
            )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_task_execution_performance(
        self, setup_performance_system
    ):
        """测试并发任务执行性能"""
        system = await setup_performance_system
        task_service = system["task_service"]
            ...
            for i in range(0, len(tasks), max_concurrent):
                ...
                for task in batch:
                    execution_context = TaskExecutionContext(
                        task_id=task.task_id, user_id="perf_test_user"
                    )
                ...
                print(
                    f"批次 {i//max_concurrent + 1}: 提交 {len(batch)} 个任务, 当前运行 {len(running_tasks)} 个"
                )
    ...
    async def test_memory_usage_under_load(self, setup_performance_system):
        ...
        for i in range(num_tasks):
                ...
                automation_config={
                    ...
                    "actions": [
                        {"type": "detect_window", "window_title": f"窗口_{i+1}"},
                        {
                            ...
                            "save_path": f"screenshot_{i+1}.png",
                        },
                        {
                            "type": "find_element",
                            "element_type": "button",
                            "element_text": f"按钮_{i+1}",
                        },
                    ]
                    * 5,  # 增加动作数量
                },
            ...
            task = await task_service.create_task(
                user_id="memory_test_user", config=config
            )
            tasks.append(task)
            ...
            if (i + 1) % 100 == 0:
                ...
                print(
                    f"创建 {i+1} 个任务后内存使用: {current_memory:.2f} MB (+{memory_increase:.2f} MB)"
                )
