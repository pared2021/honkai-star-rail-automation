...
class TestTaskManagerImprovements:
    ...
    async def test_async_crud_operations(self):
        ...
        task_id = await self.task_manager.create_task(config=task_config)
        ...
        assert (
            task.name == "测试任务"
        ), f"任务名称不匹配: 实际='{task.name}', 期望='测试任务'"
        print("✓ 获取任务成功")
        ...
        await self.task_manager.update_task(task_id, config=updated_config)
    ...
    async def test_data_validation(self):
        ...
        try:
            ...
            await self.task_manager.create_task(config=invalid_config)
        ...
        task_id = await self.task_manager.create_task(config=valid_config)
    ...
    async def test_transaction_support(self):
        ...
        # 测试批量状态更新
        status_updates = [
            {"task_id": task_id, "status": "running"} for task_id in task_ids
        ]
    ...
    async def test_performance_optimization(self):
        ...
        for i in range(10):
            ...
            task_id = await self.task_manager.create_task(config=config)
    ...
    async def test_scheduling_logic(self):
        ...
        high_task_id = await self.task_manager.create_task(config=high_config)
        medium_task_id = await self.task_manager.create_task(config=medium_config)
        low_task_id = await self.task_manager.create_task(config=low_config)

        task_ids = [high_task_id, medium_task_id, low_task_id]

        # 添加任务依赖关系
        self.task_manager.add_task_dependency(
