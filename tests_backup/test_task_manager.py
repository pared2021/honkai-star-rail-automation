...
class TestTaskManager(unittest.TestCase):
    ...
    def test_create_task(self):
        """测试创建任务"""
        config = TaskConfig(
            task_name="测试任务", task_type=TaskType.CUSTOM, priority=TaskPriority.HIGH
        )

        task_id = self.task_manager.create_task(
            user_id=self.test_user_id, config=config
        )
    ...
    def test_delete_task(self):
        ...
        # 创建测试任务
        config = TaskConfig(
            task_name="待删除任务", task_type=TaskType.CUSTOM, priority=TaskPriority.LOW
        )
    ...
    def test_list_tasks(self):
        ...
        configs = [
            TaskConfig(
                task_name="任务1", task_type=TaskType.CUSTOM, priority=TaskPriority.HIGH
            ),
            TaskConfig(
                task_name="任务2",
                task_type=TaskType.CUSTOM,
            ...
            TaskConfig(
                task_name="任务3", task_type=TaskType.CUSTOM, priority=TaskPriority.LOW
            ),
        ]
    ...
    def test_update_task_status(self):
        ...
        # 测试更新不存在任务的状态
        update_result = self.task_manager.update_task_status(
            "non_existent_id", TaskStatus.FAILED
        )
        self.assertFalse(update_result)
