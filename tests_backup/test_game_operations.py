...
class TestGameOperations:
    ...
    @pytest.fixture
    def game_operations(
        self, mock_game_detector, mock_action_executor, mock_config_manager
    ):
        """创建GameOperations实例"""
        return GameOperations(
            mock_game_detector, mock_action_executor, mock_config_manager
        )
    ...
    async def test_execute_daily_stamina_consumption_success(self, game_operations):
        ...
        # 模拟成功场景
        game_operations.game_detector.detect_current_scene.return_value = (
            SceneType.MAIN_MENU
        )
    ...
    async def test_execute_auto_combat_success(self, game_operations):
        """测试自动战斗执行成功"""
        config = {"auto_battle": True, "skill_priority": ["ultimate", "skill", "basic"]}
