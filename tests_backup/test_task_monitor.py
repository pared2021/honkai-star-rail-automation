...
class TestTaskMonitor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_manager = ConfigManager(
            config_dir=os.path.dirname(self.temp_config.name)
        )
        ...
        def mock_get_config(config_type):
            if config_type == ConfigType.UI_PREFERENCES:
                return {"update_interval": 1000, "max_history": 100}
            return {}
        ...
        self.automation_controller = AutomationController(
            self.config_manager, self.db_manager, self.game_detector
        )

        # 初始化任务监控器
        self.task_monitor = TaskMonitor(self.db_manager, self.automation_controller)
