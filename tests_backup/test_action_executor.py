...
class TestActionExecutor(unittest.TestCase):
    ...
    async def test_execute_wait_random(self):
        """测试随机等待"""
        action = WaitAction(
            wait_type=WaitType.RANDOM, min_duration=0.1, max_duration=0.2
        )
        result = await self.executor.execute_wait(action)
    ...
    async def test_execute_wait_condition_success(self):
        ...
        action = WaitAction(
            wait_type=WaitType.CONDITION, condition_func=condition_func, timeout=1.0
        )
        result = await self.executor.execute_wait(action)
    ...
    async def test_execute_wait_condition_timeout(self):
        ...
        action = WaitAction(
            wait_type=WaitType.CONDITION, condition_func=condition_func, timeout=0.1
        )
        result = await self.executor.execute_wait(action)
    ...
    async def test_execute_loop_count(self):
        """测试次数循环"""
        actions = [{"type": "wait", "params": {"wait_type": "fixed", "duration": 0.01}}]
    ...
    async def test_execute_loop_condition(self):
        ...
        loop_action = LoopAction(
            loop_type=LoopType.CONDITION, condition_func=condition_func, actions=actions
        )
        result = await self.executor.execute_loop(loop_action)
    ...
    def test_is_action_safe(self):
        """测试操作安全检查"""
        # 安全的点击操作
        safe_action = {"type": "click", "params": {"x": 100, "y": 200}}
        self.assertTrue(self.executor.is_action_safe(safe_action))

        # 不安全的点击操作（超出屏幕范围）
        with patch(
            "src.core.action_executor.pyautogui.size", return_value=(1920, 1080)
        ):
            unsafe_action = {"type": "click", "params": {"x": 3000, "y": 2000}}
