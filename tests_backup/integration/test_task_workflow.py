...
from src.core.sync_adapter import CallbackManager, EventBridge, SyncCallback
from src.core.task_executor import ExecutorStatus, TaskExecutor
...
class TestTaskWorkflow:
    ...
    async def test_manual_task_creation_and_execution(self, setup_services):
        ...
        event_bus.subscribe(
            event_type=TaskEventType.TASK_CREATED, handler=event_handler
        )
        event_bus.subscribe(
            event_type=TaskEventType.TASK_STATUS_CHANGED, handler=event_handler
        )
    ...
    async def test_automation_task_workflow(self, setup_services):
        ...
        with patch.object(
            ...
            automation_service, "capture_screenshot"
        ) as mock_screenshot, patch.object(
            automation_service, "click_at_position"
        ) as mock_click:
    ...
    async def test_task_dependency_workflow(self, setup_services):
        ...
        parent_task_id = await task_service.create_task(
            parent_create_request, "test_user"
        )
        ...
        child_task_id = await task_service.create_task(
            child_create_request, "test_user"
        )
        child_task = await task_service.get_task(child_task_id, "test_user")

        # 设置任务依赖关系
        await task_service.add_task_dependency(
            child_task_id, parent_task_id, "test_user"
        )
