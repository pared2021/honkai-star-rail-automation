...
from src.core.game_detector import GameDetector, WindowManager, TemplateMatcher, GameWindow, UIElement, SceneType
...
class TestGameDetector(unittest.TestCase):
    ...
    def test_capture_screenshot_success(self):
        ...
        with patch.object(self.detector.window_manager, 'capture_window', return_value=mock_screenshot):
    ...
    def test_identify_scene_main_menu(self):
        ...
        with patch.object(self.detector.template_matcher, 'match_multiple_templates', return_value=mock_elements):
    ...
    def test_identify_scene_unknown(self):
        """测试识别未知场景"""
        mock_screenshot = np.zeros((100, 100, 3), dtype=np.uint8)
        with patch.object(self.detector.template_matcher, 'match_multiple_templates', return_value=[]):
            scene = self.detector.detect_current_scene(mock_screenshot)
    ...
    def test_find_ui_element_failure(self):
        """测试查找UI元素失败"""
        mock_screenshot = np.zeros((100, 100, 3), dtype=np.uint8)
        with patch.object(self.detector, 'capture_screenshot', return_value=mock_screenshot):
            with patch.object(self.detector.template_matcher, 'match_template', return_value=None):
    ...
    def test_is_game_active_with_window(self):
        ...
        with patch.object(self.detector, 'detect_game_window', return_value=mock_window):
    ...
    def test_get_game_status(self):
        ...
        with patch.object(self.detector.window_manager, 'find_game_window', return_value=mock_window):
...
class TestWindowManager(unittest.TestCase):
    ...
    def test_find_game_window_success(self, mock_foreground, mock_rect, mock_text, mock_visible, mock_enum):
        """测试成功查找游戏窗口"""
