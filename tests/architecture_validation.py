...
class ArchitectureValidator:
    ...
    def _validate_dependencies(self):
        ...
        result = ValidationResult(
            ...
            score=(
                ...
                else max(0, 100 - len(circular_deps) * 10)
            ),
        ...
        result = ValidationResult(
            ...
            score=(
                ...
                else max(0, 100 - len(layer_violations) * 5)
            ),
    ...
    def _validate_interfaces(self):
        ...
        result = ValidationResult(
            ...
            score=(
                ...
                else max(0, 100 - len(interface_issues) * 10)
            ),
        ...
        result = ValidationResult(
            ...
            score=(
                ...
                else max(0, 100 - len(consistency_issues) * 5)
            ),
    ...
    def _validate_solid_principles(self):
        ...
        result = ValidationResult(
            ...
            score=(
                100.0
                if len(srp_violations) == 0
                else max(0, 100 - len(srp_violations) * 5)
            ),
        ...
        result = ValidationResult(
            ...
            score=(
                100.0
                if len(ocp_violations) == 0
                else max(0, 100 - len(ocp_violations) * 10)
            ),
        ...
        result = ValidationResult(
            ...
            score=(
                100.0
                if len(dip_violations) == 0
                else max(0, 100 - len(dip_violations) * 10)
            ),
    ...
    def _validate_module_coupling(self):
        ...
        result = ValidationResult(
            ...
            score=(
                ...
                else max(0, 100 - len(high_coupling) * 15)
            ),
    ...
    def _validate_configuration_management(self):
        ...
        result = ValidationResult(
            ...
            score=(
                ...
                else max(0, 100 - len(config_issues) * 10)
            ),
        ...
        for py_file in self.core_path.rglob("*.py"):
            ...
            for class_name, methods in classes.items():
                if len(methods) > 20:  # 方法过多可能违反SRP
                    violations.append(
                        f"{py_file.name}::{class_name} 方法过多 ({len(methods)}个)"
                    )

                # 检查方法复杂度
                complex_methods = [
                    m for m in methods if self._calculate_method_complexity(m) > 10
                ]
                if complex_methods:
                    violations.append(
                        f"{py_file.name}::{class_name} 包含复杂方法: {complex_methods}"
                    )
    ...
    def _check_dependency_inversion(self) -> List[str]:
        ...
        for py_file in self.core_path.rglob("*.py"):
            ...
            concrete_imports = [
                imp
                for imp in imports
                if not imp.startswith("I")
                and any(
                    keyword in imp.lower()
                    for keyword in ["manager", "executor", "scheduler"]
                )
            ]

            if concrete_imports:
                violations.append(
                    f"{py_file.name} 直接导入具体实现: {concrete_imports}"
                )
    ...
    def _calculate_coupling_metrics(self) -> Dict[str, Dict[str, float]]:
        ...
        for module in dependencies:
            ...
            # 计算耦合度 (0-1之间，越低越好)
            coupling = (
                (incoming + outgoing) / (2 * total_modules) if total_modules > 1 else 0
            )
    ...
    def _check_implementation_consistency(
        self, interface: str, implementation: str
    ) -> bool:
        """检查实现一致性"""
        # 简化检查：假设如果实现类存在就是一致的
        for py_file in self.core_path.rglob("*.py"):
    ...
    def _uses_dependency_injection(self, file_path: Path) -> bool:
        ...
        try:
            content = file_path.read_text(encoding="utf-8")
            return "def __init__(self" in content and (
                "manager" in content or "executor" in content
            )
        except Exception:
            return False
...
def main():
    ...
    for result in report["detailed_results"]:
        ...
        print(
            f"  {status} [{result.category}] {result.test_name}: {result.score:.1f}/100"
        )
        if result.details:
