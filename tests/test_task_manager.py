...
class TestTaskManager:
    ...
    def setup_method(self):
        ...
        with patch("src.core.task_manager.TaskExecutor") as mock_executor:
            ...
            self.task_manager = TaskManager(
                db_manager=self.mock_db_manager, default_user_id="test_user"
            )
    ...
    def test_init_with_custom_params(self):
        ...
        with patch("src.core.task_manager.TaskExecutor"):
            tm = TaskManager(db_manager=mock_db_manager, default_user_id="custom_user")
    ...
    async def test_create_task_with_args(self):
        ...
        with patch.object(
            self.task_manager, "create_task", return_value="task_456"
        ) as mock_create:
            task_id = await self.task_manager.create_task(
                func=test_func,
                args=(1, 2),
    ...
    async def test_create_async_task(self):
        ...
        with patch.object(
            self.task_manager, "create_task", return_value="async_task_789"
        ) as mock_create:
            task_id = await self.task_manager.create_task(
                func=async_test_func, name="async_test_task"
            )
    ...
    def test_submit_task(self):
        ...
        with patch.object(
            self.task_manager, "submit_concurrent_task", return_value="exec_123"
        ) as mock_submit:
    ...
    async def test_get_task_status(self):
        """测试获取任务状态"""
        task_id = "task_123"
        expected_status = {"id": task_id, "state": TaskState.RUNNING, "progress": 50}

        with patch.object(
            self.task_manager, "get_task", return_value=expected_status
        ) as mock_get:
    ...
    def test_get_task_result(self):
        ...
        with patch.object(
            self.task_manager, "get_task", return_value={"result": expected_result}
        ) as mock_get:
            # 模拟从任务信息中获取结果
    ...
    def test_cancel_task(self):
        ...
        with patch.object(
            self.task_manager, "cancel_concurrent_task", return_value=True
        ) as mock_cancel:
    ...
    def test_cancel_running_task(self):
        ...
        with patch.object(
            self.task_manager, "get_task", return_value={"state": TaskState.RUNNING}
        ) as mock_get:
            with patch.object(
                self.task_manager, "cancel_concurrent_task", return_value=True
            ) as mock_cancel:
    ...
    def test_resume_task_execution(self):
        ...
        with patch.object(
            self.task_manager, "get_task", return_value={"state": TaskState.PAUSED}
        ) as mock_get:
            # 验证可以从暂停状态恢复
            task = mock_get.return_value
    ...
    async def test_list_tasks(self):
        ...
        with patch.object(
            self.task_manager, "list_tasks", return_value=mock_tasks
        ) as mock_list:
    ...
    async def test_get_task_statistics(self):
        ...
        with patch.object(
            self.task_manager, "get_task_statistics", return_value=mock_stats
        ) as mock_get_stats:
...
class TestTaskManagerIntegration:
    ...
    def test_full_task_lifecycle(self):
        ...
        with patch.object(
            ...
            self.task_manager, "get_concurrent_status"
        ) as mock_status:
