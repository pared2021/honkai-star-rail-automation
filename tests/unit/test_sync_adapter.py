...
class TestCallbackData(unittest.TestCase):
    ...
    def test_init_default(self):
        """测试默认初始化。"""
        data = CallbackData(task_id="test_task", callback_type=CallbackType.SUCCESS)
...
class TestSyncAdapter(unittest.TestCase):
    ...
    def test_start(self):
        ...
        self.assertEqual(self.adapter.get_status(), AdapterStatus.RUNNING)
        self.assertIsNotNone(self.adapter._loop)
        self.assertIsNotNone(self.adapter._loop_thread)
        self.assertIsNotNone(self.adapter._callback_thread)
    ...
    def test_register_callback_success(self):
        ...
        callback_id = self.adapter.register_callback(
            callback.on_success, CallbackType.SUCCESS
        )
    ...
    def test_register_callback_with_task_id(self):
        ...
        callback_id = self.adapter.register_callback(
            callback.on_success, CallbackType.SUCCESS, task_id="test_task"
        )
