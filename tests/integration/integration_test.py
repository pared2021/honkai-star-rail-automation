...
def test_core_imports():
    ...
    try:
        ...
        # 测试数据模型导入
        from models.unified_models import ExecutionConfig, ScheduleConfig, TaskConfig
...
def test_task_config_creation():
    ...
    try:
        ...
        exec_config = ExecutionConfig(
            max_retry_count=3, timeout_seconds=300, parallel_execution=False
        )

        # 创建调度配置
        schedule_config = ScheduleConfig(enabled=False, schedule_time="00:00")
...
async def test_repository_operations():
    ...
    try:
        ...
        task = Task(task_id="test_task_001", user_id="user_001", config=task_config)
...
async def main():
    ...
    # 异步测试
    async_tests = [test_repository_operations, test_service_operations]
