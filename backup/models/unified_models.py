...
from core.enums import TaskStatus, TaskType, TaskPriority, ActionType
...
class BaseModel(ABC):
    ...
    def to_json(self) -> str:
        """转换为JSON字符串"""
        return json.dumps(self.to_dict(), ensure_ascii=False, indent=2, default=self._json_serializer)
...
class Task(BaseModel):
    ...
    config: TaskConfig = field(default_factory=lambda: TaskConfig(name="", task_type=TaskType.CUSTOM))
    ...
    def update_status(self, new_status: TaskStatus, error_message: str = None):
        ...
        if error_message:
            ...
            self.execution_log.append(f"[{datetime.now().isoformat()}] ERROR: {error_message}")
