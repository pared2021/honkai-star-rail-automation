...
// 初始化服务实例（在路由处理器中动态创建）
function createServices(dbService) {
...
router.post('/executions/start', async (req, res) => {
    try {
        const { strategyId, accountId, options } = req.body;
...
// 更新执行步骤
router.put('/executions/:id/step', async (req, res) => {
    try {
        const { id } = req.params;
...
// 获取反馈列表
router.get('/feedback/:strategyId', async (req, res) => {
    try {
...
// ==================== 智能评估相关接口 ====================
// 获取智能评估结果
router.get('/evaluations/:strategyId', async (req, res) => {
    try {
        const { strategyId } = req.params;
        const dbService = req.dbService;
        const { performanceCollector, executionTracker, intelligentEvaluator } = createServices(dbService);
...
// 触发实时分析
router.post('/evaluations/analyze', async (req, res) => {
    try {
        const { strategyIds, analysisType } = req.body;
        ...
        const dbService = req.dbService;
        const { performanceCollector, executionTracker, intelligentEvaluator } = createServices(dbService);
        const results = await Promise.all(strategyIds.map(id => intelligentEvaluator.evaluateStrategy(id)));
        res.json({
            success: true,
            data: results,
            message: '实时分析完成'
...
// 获取性能仪表板数据
router.get('/performance/dashboard/:strategyId', async (req, res) => {
    try {
        const { strategyId } = req.params;
...
// 获取实时性能数据
router.get('/performance/realtime/:sessionId', async (req, res) => {
    try {
...
// 获取性能历史数据
router.get('/performance/history/:strategyId', async (req, res) => {
    try {
        const { strategyId } = req.params;
...
// ==================== 任务信息管理相关接口 ====================
// 获取任务信息列表
router.get('/task-info', async (req, res) => {
    try {
...
// 根据ID获取任务信息
router.get('/task-info/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const dbService = req.dbService;
...
// 更新任务信息
router.put('/task-info/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
// 删除任务信息
router.delete('/task-info/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const dbService = req.dbService;
...
// 搜索任务信息
router.get('/task-info/search', async (req, res) => {
    try {
        ...
        const dbService = req.dbService;
        const searchResults = await dbService.searchTaskInfos(query, filters ? JSON.parse(filters) : {});
        res.json({
            success: true,
            data: searchResults
        });
...
// 复制攻略
router.post('/strategies/:id/clone', async (req, res) => {
    try {
        const { id } = req.params;
        const { name } = req.body;
        const dbService = req.dbService;
        ...
        const clonedStrategy = await dbService.cloneStrategy(id, name || `${originalStrategy.name || originalStrategy.strategyName || 'Unknown Strategy'} (副本)`);
        res.status(201).json({
            success: true,
            data: clonedStrategy,
            message: '攻略复制成功'
...
// 获取攻略步骤
router.get('/strategies/:id/steps', async (req, res) => {
    try {
        const { id } = req.params;
        const dbService = req.dbService;
...
// 更新攻略步骤
router.put('/strategies/:id/steps', async (req, res) => {
    try {
        const { id } = req.params;
...
// 批量评估攻略
router.post('/strategies/batch-evaluate', async (req, res) => {
    try {
...
// 比较攻略性能
router.post('/strategies/compare', async (req, res) => {
    try {
...
// 获取攻略优化建议
router.get('/strategies/:id/recommendations', async (req, res) => {
    try {
        const { id } = req.params;
        const dbService = req.dbService;
        const strategyAnalyzer = req.strategyAnalyzer;
...
// 获取攻略成功率统计
router.get('/strategies/:id/success-rate', async (req, res) => {
    try {
        const { id } = req.params;
        const { timeRange } = req.query;
        const dbService = req.dbService;
        const successRate = await dbService.getStrategySuccessRate(id, timeRange);
        res.json({
            success: true,
            data: successRate
        });
...
// 获取调度配置
router.get('/automation/schedule', async (req, res) => {
    try {
...
// 更新调度配置
router.put('/automation/schedule/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const updateData = req.body;
        const dbService = req.dbService;
...
// 删除调度配置
router.delete('/automation/schedule/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const dbService = req.dbService;
...
// 启用/禁用调度
router.patch('/automation/schedule/:id/toggle', async (req, res) => {
    try {
        const { id } = req.params;
...
// 获取系统配置
router.get('/automation/system-config', async (req, res) => {
    try {
        const dbService = req.dbService;
...
// 更新系统配置
router.put('/automation/system-config', async (req, res) => {
    try {
        const configData = req.body;
        const dbService = req.dbService;
...
router.get('/automation/status', async (req, res) => {
    try {
        ...
        const status = await automationManager.getAutomationStatus(accountId);
        res.json({
            success: true,
            data: status
        });
